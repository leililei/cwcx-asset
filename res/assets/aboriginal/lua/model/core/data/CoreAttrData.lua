...
function CoreAttrData:ctor()
    ...
    self.type = 0       ---属性是否为百分比(0.值，1.百分比)
    ...
    CoreAttrData.coreAttrName = {
        ...
        [7] = MgrLanguageData.GetLanguageByKey("ui_yangcheng_text19"),
        [8] = MgrLanguageData.GetLanguageByKey("coreattrdata_support"),
        [26] = MgrLanguageData.GetLanguageByKey("ui_yangcheng_text19"),
    }
end
local enum = {
    value = 0,      ---值
...
function CoreAttrData:AddData(id,attr)
    if self.attrID == id then
        self.attribute = self.attribute + attr
    end
end
