...
---保存装备锁状态
function CoreControl.SaveCoreLock(CoreUid,lock,callBack)
    BagViewModel.ChangeLockReq(CoreUid,lock,callBack)
end
---读取装备锁状态
function CoreControl.GetCoreLock(CoreUid)
    ---以装备Uid来保存装备是否上锁
    return CoreControl.GetSingleCoreData(CoreUid).isLocked
end
---删除装备锁数据
