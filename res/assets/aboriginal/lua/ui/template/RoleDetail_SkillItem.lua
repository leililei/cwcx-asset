...
local M = Class('RoleDetail_SkillItem', UIItemBase)
function M:ctor()
    M.super.ctor(self)
    self.PathPrefab = 'ABOriginal/UI/Prefab/Template/RoleDetail_SkillItem.prefab'
    -- 没有使用组建缓存列表
    self.CC = {
        -- Image 列表
        {'Img_Jinengdi','Img_Jinengdi',2},{'TagPanel','TagPanel',2},{'Img_Tag','Img_Tag',2},{'EX','EX',2},{'Img_Jinengdian_hui','Img_Jinengdian_hui',2},{'Img_Jinengdian2','Img_Jinengdian2',2},{'Img_Jinengdian1','Img_Jinengdian1',2},{'JinengdianPanel','JinengdianPanel',2},{'Img_Suo','Img_Suo',2},
        -- UITemplate 列表
        {'RoleDetail_SkillItem','/',10},
        -- TextMeshProUGUI 列表
        {'Text_Tag','Img_Tag/Text_Tag',20},{'Text_Tag01','EX/Text_Tag',20},{'Text_Jinengming','Text_Jinengming',20},{'Text_Jinengshuoming','Text_Jinengshuoming',20},{'Text_Jiesuotiaojian','Img_Suo/Text_Jiesuotiaojian',20},
    }
end
...
function M:OnUpdateUI(pData)
    ...
    if self.pData.SkillNum ~= 5 then
        ...
        for i, v in ipairs(self.pData.SkillLvStage) do
            if self.CurHeroAttr.ShowSkillLV >= v then
                if tIsEquipReach then
                    self:CreatObjInPanel(self.JinengdianPanel(),self.Img_Jinengdian2())
                else
    ...
    if tIsOpen then     ---技能已解锁
        ...
        if pData[4] ~= nil and pData[4] == "Battle" then
            self.JinengdianPanel().gameObject:SetActive(false)
        end
        ---标签
    ...
    self.XiangqingData = {
        ...
        tagList = tTagList,
        curSkillLv = self.CurHeroAttr.SkillLV
    }
end
function M:OnClickSelf()
    if self.pData ~= nil and self.CanClick then
        self.parent:ShowSkillXiangqing(self.pData,self.ObjRoot,self.XiangqingData)
    end
end
