...
--------跳转管理---------
function MgrLink.LinkStart(_LinkMark, callBack)
    ...
    elseif _LinkMark[2] == "sy" then
        if _LinkMark[3] == "zjm" then
            if SysLockControl.CheckSysLock(1110) then
                YueTaViewModel.OpenYuetaUI()
            else
                MgrUI.Pop(UID.PopTip_UI, { SysLockControl.GetSystemLockTips(1110), 1}, true)
            end
        end
    ...
    elseif _LinkMark[2] == "sc" then
        ...
        elseif _LinkMark[3] == "wzbc" then
            ...
            ShopViewModel.OpenShopUI()
        elseif _LinkMark[3] == "zbsd" then
            ---皮肤商城
            ShopViewModel.GetItemsDisCountTime(function(shopList)
                ShopControl.InitAllShopTime(shopList)
                MgrUI.GoHide(UID.ShopSkin_UI)
                if callBack then
                    callBack()
                end
            end)
        ...
        elseif _LinkMark[3] == "tj" then
            ShopViewModel.JumpToRecommend(CheckNumber(_LinkMark[4]))
        elseif tonumber(_LinkMark[3]) ~= nil then
            ...
            if shoptypeCfg then
                if SysLockControl.CheckSysLock(shoptypeCfg.systemopen) then
                    ShopViewModel.WhetherJumpIn = tonumber(_LinkMark[3]) ~= 114001
    ...
    elseif _LinkMark[2] == "hd" then
        ...
        elseif _LinkMark[3] == "wudaohui" then
            ---宇宙武道会主界面
            BuDoukaiControl.OpenHome()
        elseif _LinkMark[3] == "grjq" then
            ---个人剧情
    ...
    elseif _LinkMark[2] == "sbtm" then
        if _LinkMark[3] == "zjm" then
            ---活动主界面
            Event4Control.OpenEvent4UI()
        end
    elseif _LinkMark[2] == "half" then
        ---半周年
        if _LinkMark[3] == "zjm" then
...
function MgrLink.TurnToStormPointPage(plotIndex, stormIndex)
    ...
    StormViewModel.ReloadStormData()
    if isLock then
        StormViewModel.RequestLevelDropInfo(pData.points, function() 
