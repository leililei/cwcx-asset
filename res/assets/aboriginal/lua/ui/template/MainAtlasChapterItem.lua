...
local M = Class('MainAtlasChapterItem', UIItemBase)
function M:ctor()
    M.super.ctor(self)
    self.PathPrefab = 'ABOriginal/UI/Prefab/Template/MainAtlasChapterItem.prefab'
    -- 没有使用组建缓存列表
    self.CC = {
        -- Image 列表
...
function M:OnUpdateUI(Args)
    ...
    ---序号
    self.Text_Number().text = self.data.chapterid >= 10 and self.data.chapterid or "0"..self.data.chapterid
    ---红点
    ...
    if self.data.checkpointid == 0 then
        ...
        self.Panel_Lock().gameObject:SetActive(not StormControl.CheckPointPass(self.data.checkpointid))
    end
    if StormControl.CheckPointPass(self.data.checkpointid) == false then
        self.LockTips().text = StormControl.GetStormPointByID(self.data.checkpointid).name
    end
    self.Unlock = StormControl.CheckPointPass(self.data.checkpointid)
end
