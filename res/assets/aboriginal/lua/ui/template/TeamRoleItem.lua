...
function M:OnUpdateUI(pData)
    ...
    self.pData = pData[1]
    self.parent = pData[2]
    if  next(self.pData) == nil then
        self.NullPanel().gameObject:SetActive(true)
        self.Panel().gameObject:SetActive(false)
    else
        self.NullPanel().gameObject:SetActive(false)
        ...
        if self.pData.awaken then
            for i = 0,self.awaken.transform.childCount - 1 do
                self.awaken.transform:GetChild(i).gameObject:SetActive(false)
                if self.pData.star >= i+1 then
                    self.awaken.transform:GetChild(i).gameObject:SetActive(true)
                end
            end
        end
        ...
        if self.pData.heroFlag ~= nil and self.pData.heroFlag ~= 0 and self.pData.heroFlag ~= false and self.pData.heroCurSkill < self.pData:GetHeroShowSkillLv() then
            if self.pData:CheckHeroEquipIsMax() then
                self.Text_Rank().text = "<color=#FFB800>" .. self.pData.heroCurSkill .."</color>"
            else
                self.Text_Rank().text = "<color=#FFFFFF>" .. self.pData.heroCurSkill .."</color>"
            end
        else
            ---获取技能等级
            if self.pData:CheckHeroEquipIsMax() then
                self.Text_Rank().text = "<color=#FFB800>" .. self.pData:GetHeroSkillLevel() .."</color>"
            else
                self.Text_Rank().text = "<color=#FFFFFF>" .. self.pData:GetHeroSkillLevel() .."</color>"
            end
        end
    end
end
