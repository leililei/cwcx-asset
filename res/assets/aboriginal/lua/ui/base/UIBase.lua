...
function UIBase:AddLRPanel()
    local go = GameObject()
    go.transform:SetParent(self.ObjRoot.transform)
    local image = go:AddComponent(typeof(UnityEngine.UI.Image))
    MgrRes.LoadSprite(image,"UIMusk/Img_Heibian_you",nil,true)
    ...
    local canvas1 = go:AddComponent(typeof(UnityEngine.Canvas))
    canvas1.overrideSorting = true
    canvas1.sortingOrder = 32767
    go.layer = 5

    local go2 = GameObject()
    go2.transform:SetParent(self.ObjRoot.transform)
    local image2 = go2:AddComponent(typeof(UnityEngine.UI.Image))
    MgrRes.LoadSprite(image2,"UIMusk/Img_Heibian_zuo",nil,true)
    local rect2 = go2:GetComponent("RectTransform")
    rect2.anchorMax = Vector2(0, 0.5)
    rect2.anchorMin = Vector2(0, 0.5)
    rect2.pivot = Vector2(1, 0.5)
    rect2.anchoredPosition = Vector2(0, 0)
    local canvas2 = go2:AddComponent(typeof(UnityEngine.Canvas))
