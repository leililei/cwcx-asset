...
function SummerPreheaData:ctor(id)
    ...
    self.open_time = "0"       ---开启时间
    self.end_time = "0"       ---开启时间
    local timeC = TimeControl.GetTimeTable(tonumber(config.open_time))
    if timeC then
        self.open_time = Global.TimeToDate(timeC.openTime)       
        self.end_time = Global.TimeToDate(timeC.endTime)         
    end
    self.picIdA = "0"                       ---角色A
    ...
    self.music = "0"                        ---背景音乐
    self.awards = nil                       ---当天是否发奖
    self.dialogId = config.dialogueid       ---对话表ID
