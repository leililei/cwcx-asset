...
BattleRole.specialEquip = nil
BattleRole.isSummonObj = false
--头像图标
BattleRole.Icon=nil
...
BattleRole.DamageRelief = 0

function BattleRole:SetDamageRelief(_role,_SkillNum)
    if _SkillNum < 0 then
        _SkillNum = 0
    end
    _role.DamageRelief = _SkillNum
end
...
function BattleRole:RoleAniLoad(tempRole,delay,func)
    ...
    if not func then
        --print("BattleRole:RoleAniLoad. GameID: " .. tempRole.GameID .. ", AniName: " .. tempRole.AniName);
        CBattleTools.CCreatGo(tempRole.GameID,tempRole.AniName,tempRole.PosX,tempRole.PosY,tempRole.IsLeft,0,tempRole.HP,tempRole.Qzoom,(tempRole.SkinID == nil and tempRole.ID or tempRole.SkinID),tempRole.HPmax, function(myAni)
        ...
        CBattleTools.CCreatGo(tempRole.GameID,tempRole.AniName,tempRole.PosX,tempRole.PosY,tempRole.IsLeft,0,tempRole.HP,tempRole.Qzoom,(tempRole.SkinID == nil and tempRole.ID or tempRole.SkinID),tempRole.HPmax, func)
    end
end
...
BattleRole.tryHeroUID = nil;
BattleRole.tryHeroProps = {};

function BattleRole:SyncTryHero(roleData)
    -- roleData 类型为 RoleData
    self.tryHeroUID = roleData.tryHeroUID;
    self.tryHeroProps = {};
    if nil == roleData.tryHeroProps then
        return;
    end
    self.tryHeroProps = roleData.tryHeroProps;
    for i, v in ipairs(self.tryHeroProps) do
        if v.field == PropEnum.Level then
            self.LV = v.value
        elseif v.field == PropEnum.Rank then
            self.SkillLV = v.value
        elseif v.field == PropEnum.Star then
            self.StartLV = v.value
        ...
        elseif v.field == PropEnum.Support then
            self.RealSuppart = v.value / 10000;
        end
    end
end
