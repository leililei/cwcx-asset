...
function CoreData:ctor()
    ...
    self.isLocked = 0               ---核心是否已上锁 1 锁定 0 未锁定
    self.armorGroup = {}            ---核心装备信息(新增防守类型group：0攻击 1防守)
end
-------------数据操作方法-------------
...
function CoreData:PushData(armors)
    ...
    self.isLocked = armors.armorStatus
    print("强化后最高经验",self.maxExp)
    self.armorGroup = armors.armorGroup ~= nil and armors.armorGroup or {}
end
---配置数据
...
function CoreData:ReLoadCore(type, roleId, armorGroup)
    self.slot = type
    self.RoleId = roleId
    
    if roleId == 0 then
        for i = 1, #self.armorGroup do
            if self.armorGroup[i].group == armorGroup then
                table.remove(self.armorGroup,i)
                break
            end
        end
    else
        ...
        table.insert(self.armorGroup, armor)
    end
end
