...
local M = Class('ShopGiftItem', UIItemBase)
function M:ctor()
    M.super.ctor(self)
    self.PathPrefab = 'ABOriginal/UI/Prefab/Template/ShopGiftItem.prefab'
    -- 没有使用组建缓存列表
    self.CC = {
        -- Image 列表
        {'ShopGiftItem','/',2},{'ShopItemPanel','ShopItemPanel',2},{'Img_Icon','ShopItemPanel/Img_Icon',2},{'Img_Name','ShopItemPanel/Img_Name',2},{'Img_FrameB','ShopItemPanel/Img_Name/Img_FrameB',2},{'Img_Count','ShopItemPanel/Img_Count',2},{'Img_DisCount','ShopItemPanel/Img_DisCount',2},{'Btn_Goumai','ShopItemPanel/Btn_Goumai',2},{'Img_PriceIconZ','ShopItemPanel/Btn_Goumai/Panel/Img_PriceIconZ',2},{'Panel','ShopItemPanel/Btn_Goumai/Panel/Text_Original/Panel',2},{'Img_SoldOut','Img_SoldOut',2},
        -- UITemplate 列表
        {'ShopGiftItem01','/',10},
        -- TextMeshProUGUI 列表
        {'Text_Name','ShopItemPanel/Img_Name/Text_Name',20},{'Text_Count','ShopItemPanel/Img_Count/Text_Count',20},{'Text_DisCount','ShopItemPanel/Img_DisCount/Text_DisCount',20},{'Text_ReTime','ShopItemPanel/Text_ReTime',20},{'Text_Price','ShopItemPanel/Btn_Goumai/Panel/Text_Price',20},{'Text_Original','ShopItemPanel/Btn_Goumai/Panel/Text_Original',20},{'Text_CantBuy','Img_SoldOut/Text_CantBuy',20},
    }
end
-- Code Auto Create End
require("LocalData/PaidgoodsLocalData")
function M:OnInit()
    UIEvent.LuaClick(self.ShopItemPanel().gameObject,function()
        ---为0表示没有解锁条件
            ...
            MgrUI.Pop(UID.BuyPop_UI,{self.pData,function(count)
                ...
                if self.pData.shopType == 400000 then
                    ...
                    ShopViewModel.SendBuyGoods({self.pData.id,1},function()
                        --self.parent.LastChildShop = 0
                        if self.pData.shopType2 == 100 or self.pData.shopType2 == 102  then
                            ---刷新首推商店
                            Event.Go("ShopHome_UI_ReloadScroll")
                            ---刷新货币
...
function M:OnUpdateUI(args)
    ...
    MgrRes.LoadSprite(self.ShopItemPanel(),string.format("Quality/ShopGift_%s", self.pData.rank))

    if self.pData.shopType2 == 100 then
        local price
        local priceGoods = self.pData:GetPrice()
        local priceIcon = string.format("Item/%s",Global.GetLocalDataByGoods(priceGoods).icon)
        MgrRes.LoadSprite(self.Img_PriceIconZ(),priceIcon)
        ---策划需求 100008商品直接读数目
