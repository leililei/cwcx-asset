...
---服务器错误码专用
function MgrLanguageData.GetErrorByKey(_key)
    if MgrLanguageData.LanguageDatas["errorcode_".._key] ~= nil then
        return MgrLanguageData.LanguageDatas["errorcode_".._key].LangChs
    end
    --print("多语言无索引".._key)
    local tStr = string.format(MgrLanguageData.GetLanguageByKey("errorcode_zero"),_key)
    return tStr
end
---多语言类型
