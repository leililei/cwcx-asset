...
function PlayerData:ctor()
    ...
    self.monthSign = 0          ---月签到天数
    self.monthSignFlag = true
    ...
    self.limitSign = nil
    self.HomeRole = nil         --主界面看板娘角色
    self.regression = nil       --回归签到
    self.newMonthCards = {};
end
---覆盖玩家数据
function PlayerData:PushData(info)
    ...
    self:PushMonthSignData(info.mSign)  --月签到天数
    self:PushMonthSignBool(info.mSignFlag)  --今天是否月签到
    ...
    if info.menuRoleID == 0 then
        if #info.heros == 0 then
            ...
            if #tStr > 0 then
                ...
                self.HomeRole = tonumber(tStr[1])
            end
        else
            self.curRoleID = info.heros[1].heroID
            self.HomeRole = info.heros[1].heroID
        end
    else
        self.curRoleID = info.menuRoleID
        self.HomeRole = info.menuRoleID
    end
    ...
    self.regression = info.regression
...
function PlayerData:PushMonthSignBool(mSignFlag)
    self.monthSignFlag = mSignFlag
end
---更新体力
...
function PlayerData:GetNewMonthCardRemaining(id)
    if nil == self.newMonthCards then
        return 0;
    end
    for _, v in pairs(self.newMonthCards) do
        if id == v.cardID then
            return v.remaining;
        end
    end
...
function PlayerData:MinusNewMonthCardRemaining(id)
    if nil == self.newMonthCards then
        return 0;
    end
    for _, v in pairs(self.newMonthCards) do
