...
local M = Class('GiftShopItem', UIItemBase)
function M:ctor()
    M.super.ctor(self)
    self.PathPrefab = 'ABOriginal/UI/Prefab/Template/GiftShopItem.prefab'
    -- 没有使用组建缓存列表
    self.CC = {
        -- Image 列表
        {'GiftShopItem','/',2},{'ShopItemPanel','ShopItemPanel',2},{'Img_Di','ShopItemPanel/Img_Di',2},{'Img_Frame','ShopItemPanel/Img_Frame',2},{'Img_Icon','ShopItemPanel/Img_Icon',2},{'ReTime','ShopItemPanel/ReTime',2},{'Img_PriceIcon','ShopItemPanel/Panel/Img_PriceIcon',2},{'Panel','ShopItemPanel/Panel/Text_Original/Panel',2},{'Img_Count','ShopItemPanel/Img_Count',2},{'Img_DisCount','ShopItemPanel/Img_DisCount',2},{'Img_Name','ShopItemPanel/Img_Name',2},{'Img_FrameB','ShopItemPanel/Img_Name/Img_FrameB',2},{'StarPanel','ShopItemPanel/StarPanel',2},{'ItemStarPrefab','ShopItemPanel/StarPanel/ItemStarPrefab',2},{'HighLight','ShopItemPanel/StarPanel/ItemStarPrefab/HighLight',2},{'ItemStarRoot','ShopItemPanel/StarPanel/ItemStarRoot',2},{'Img_SoldOut','Img_SoldOut',2},{'RedDot','ShopItemPanel/RedDot',2},
        -- UITemplate 列表
        {'GiftShopItem01','/',10},
        -- TextMeshProUGUI 列表
    ...
    UIEvent.LuaClick(self.Img_Di().gameObject,function()
        ...
        if self.pData.isSellGroup then
            ...
            MgrUI.Pop(UID.BuyPop_UI,{self.pData,function(count)
                ...
                if table.Contains(ShopControl.ConstData.CASH_CHILD_SHOP_TYPES, self.pData.shopType2) then
                ...
                if self.pData.shopType == 400000 then
                    ...
                    ShopViewModel.SendBuyGoods({self.pData.id,1},function()
                        --self.parent.LastChildShop = 0
                        if table.Contains(ShopControl.ConstData.CASH_CHILD_SHOP_TYPES, self.pData.shopType2) or self.pData.shopType2 == 150 then
                            ---刷新首推商店
                            Event.Go("ShopHome_UI_ReloadScroll")
                            ---刷新货币
                            Event.Go("Shop_UI_ChangeWallet")
                        else
                            if self.parent.RefreshCurShop ~= nil then
                                self.parent:RefreshCurShop()
                            end
                        end
                    end)
...
function M:OnUpdateUI(args)
    ...
    local pathString = string.format("Quality/img_libaopinzhi_%s", self.pData.rank)
    MgrRes.LoadSprite(self.Img_Frame(),pathString)
    ...
    self.Text_ReTime().gameObject:SetActive(true)
    ...
    if self.pData.shopType2 == 101 then
        for i,v in pairs(HomeViewModel.packageList) do
            if v.ID == self.pData.id then
                if v.Duration == -1 then
                    self.ReTime().gameObject:SetActive(false)
                    self.Text_ReTime().gameObject:SetActive(false)
                end
            end
        end
    end
    ...
    if self.pData.shopType2 == 102 then
        bShowRedDot = self.pData:IsShowRedDot()
    end
    if self.RedDot() then
        self.RedDot().gameObject:SetActive(bShowRedDot)
    end
end
