...
function M:ctor()
    ...
    self.CC = {
        ...
        -- TextMeshProUGUI 列表
        {'RoleVoiceWordText','Ani/Role/Img_SoundTextBG/RoleVoiceWordText',20},{'Text_name','Ani/RIghtGroup/CommodityGroup/Mask/CommodityItem/Text_name',20},{'Text_ShengYu','Ani/RIghtGroup/CommodityGroup/Mask/CommodityItem/Text_ShengYu',20},{'Text_JiaGe','Ani/RIghtGroup/CommodityGroup/Mask/CommodityItem/CoinPlane/Text_JiaGe',20},{'Text_CantBuy','Ani/RIghtGroup/CommodityGroup/Mask/CommodityItem/Img_SoldOut/Text_CantBuy',20},{'Text_ZSCount','Ani/Btn_Coin/Text_ZSCount',20},{'Text_Title_CN','Ani/ReturnBg/Text_Title/Text_Title_CN',20},{'Text_Title_EN','Ani/ReturnBg/Text_Title/Text_Title_EN',20},
    }
end
...
function M:OnInit()
    ...
    local tCurRoleId = 0
    for i, v in ipairs(self.HaiyueData.shopType) do
        tCurRoleId = ShoptypeLocalData.tab[v].npcid
        --添加音效
        MgrSound.AddCue("Audio/role/".. tCurRoleId..".acb")
    end

    if #self.HaiyueData.shopType == 1 then
        --self.TopGroup().gameObject:SetActive(false)
        self.Shop2().gameObject:SetActive(false)
    end
    ---弹出引导帮助界面
    ...
    ---背景音乐
    if self.HaiyueData and self.HaiyueData ~= "" then
...
function M:InitButton()
    ...
    ---点击第一个商店
    local tog1Normal = self.Shop101().gameObject.transform:Find("Unchecked").gameObject
    local tog1HL = self.Shop101().gameObject.transform:Find("Checked").gameObject
    Tools.ToggleValueChange(self.Shop101(),function(isOn)
        tog1Normal:SetActive(not isOn)
        tog1HL:SetActive(isOn)
        if isOn then
            HaiYueControl.curShopType = self.HaiyueData.shopType[1]
    ...
    ---点击第二个商店
    local tog2Normal = self.Shop201().gameObject.transform:Find("Unchecked").gameObject
    local tog2HL = self.Shop201().gameObject.transform:Find("Checked").gameObject
    Tools.ToggleValueChange(self.Shop201(),function(isOn)
        tog2Normal:SetActive(not isOn)
        tog2HL:SetActive(isOn)
        if isOn then
            HaiYueControl.curShopType = self.HaiyueData.shopType[2]
            self:RefreshCurShop(0)
            --self:GetCurWords(97)
        end
    end)
...
function M:RefreshCurShop(offset)
    self.shopData = HaiYueControl.GetShopData()
    self.shopItemList = {}
    ---取出对应商店类型的数据
    for k,v in pairs(self.shopData[HaiYueControl.curShopType]) do
...
function M:InitShopLock()
    ...
    if (shop1UnLock and not shop2UnLock) or (shop1UnLock and shop2UnLock) then
        ---默认显示第一个
        self.Shop101().isOn = false
        self.Shop101().isOn = true
        --self.Shop201().isOn = false
        local tog2Icon = self.Shop201().gameObject.transform:Find("Checked").gameObject
    ...
    elseif shop2UnLock and not shop1UnLock then
        ---默认显示第二个
        self.Shop201().isOn = false
        self.Shop201().isOn = true
        --self.Shop101().isOn = false
        local tog1Icon = self.Shop101().gameObject.transform:Find("Checked").gameObject
        tog1Icon:SetActive(false)
        
        HaiYueControl.curShopType = self.HaiyueData.shopType[2]
    end

    self.curRoleId = ShoptypeLocalData.tab[HaiYueControl.curShopType].npcid
end
...
function M:GetCurWords(type)
    local _ActorLinesType = ShoptypeLocalData.tab[HaiYueControl.curShopType].npcid   --当前台词组别
...
function M:UpdateCoin()
    local tCfg = ShoptypeLocalData.tab[HaiYueControl.curShopType]
    if not tCfg then
        return
    end
