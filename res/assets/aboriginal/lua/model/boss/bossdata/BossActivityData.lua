...
function BossActivityData:PushData(activityID)
    ...
    if config then
        ...
        if ActivityLocalData.tab[activityID] then
            local time = TimeControl.GetTimeTable(ActivityLocalData.tab[activityID][5])
            self.beginTime = Global.TimeToDate(time.openTime)
            self.endTime = Global.TimeToDate(time.endTime)
        end
        ---战斗开启时间
        for i,v in pairs(ActivityLocalData.tab) do
            if v[2] == 996 then
                local time = TimeControl.GetTimeTable(ActivityLocalData.tab[activityID][15])
                self.battleEndTime = Global.TimeToDate(time.endTime)
                break
            end
        end
