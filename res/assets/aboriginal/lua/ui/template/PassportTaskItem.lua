...
function M:OnUpdateUI(pData)
    ...
    self.parent = pData[2]
    PassportViewModel.ReceivedCallBack = Handle(self.parent,self.parent.InitPassport)
    local integral = 0
    ...
    self.Text_TaskProgress().text = value.."<color=#7A7A7A>/"..progressStr[3].."</color>"
    self.Img_TaskProgressBar().fillAmount = value/tonumber(progressStr[3])
    ---设置积分
    --for i,v in pairs(taskData.reward) do
    --    if v.goodsID == 100008 then
    --        self.Text_IntegralCount().text = v.goodsNum
    --        break
    --    end
    --end
    ---是否已完成
    if TaskControl.CheckComplete(taskData.id) or taskData.isComplete == 1 then
        ...
        if value >= tonumber(progressStr[3]) then
            ...
            if self.taskType == 4 then
                if TaskControl.ActivityDayTaskDataList[taskData.id].gotoID == "0" then
                    self.Btu_GoTo().gameObject:SetActive(false)
                end
            elseif self.taskType == 5 then
                if TaskControl.ActivityWeekTaskDataList[taskData.id].gotoID == "0" then
                    self.Btu_GoTo().gameObject:SetActive(false)
                end
            end
            self.Btu_Receive().gameObject:SetActive(false)
...
function M:ReceiveClick()
    if self.taskType == 6 then
        AchievementViewModel.TaskPerform(0,self.taskId,self)
    else
        PassportViewModel.TaskPerform(self.taskType,self.taskId)
    end
end
...
function M:TaskJump(taskData)
    local transformInfo = {}
    if self.taskType == TaskControl.DayTaskType.ACTIVITY_DAILY then
        transformInfo = string.split(TaskControl.ActivityDayTaskDataList[taskData.id].gotoID,"_")
    elseif self.taskType == TaskControl.DayTaskType.ACTIVITY_WEEKLY then
        transformInfo = string.split(TaskControl.ActivityWeekTaskDataList[taskData.id].gotoID,"_")
    elseif self.taskType == TaskControl.AchievementTaskType.PASSPORT then
        transformInfo = string.split(TaskControl.AchievementDataList[taskData.id].gotoID,"_")
    end
    ---跳转到对应界面
