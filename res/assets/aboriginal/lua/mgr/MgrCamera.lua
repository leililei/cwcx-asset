...
function MgrCamera.VirCamerasInit_New()
    local width = CS_MgrCamera:GetScreenSafeAreaWidth()
    local height = CS_MgrCamera:GetScreenSafeAreaHeight()
    local fov = 60 / (width / height)
    if fov <= 32 then
        fov = 32
    end
    CS_MgrCamera:VirCamerasInit_New(fov,fov + 5)
end
