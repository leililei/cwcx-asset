...
--- 该方法不识别“仅剩余主界面”，可能会出现不符合期望的未关闭界面。
---@deprecated 应使用 GoClose2
...
function MgrUI.GoClose2(pUid, callback)
    _newUICell = nil
    if callback then
        _newUICell = callback
    end
    ...
    local call = function()
        local oldUI = _GetCurUI()
        --只有主界面的情况下，主界面将不被Close
        if _HasOnlyOneUI() and ((not oldUI) or oldUI.Uid == UID.Home_UI) then
            return
        end
        oldUI:Close()
        table.remove(_UIList, _GetUINum())
...
function MgrUI.CloseAllPop()
    ...
    for idx, pop in pairs(_PopList) do
        if pop.Layer == UILayerLv.Pop and pop.Uid.Name ~= "SystemNotice_UI" and pop.Uid.Name ~= "NewRoleSkill_UI" and pop.Uid.Name ~= "NewRoleFormation_UI" and (not MgrUI.IsInHideList(pop.Uid.Name)) then
            pop:Close()
        else
...
function MgrUI.IsPopOpen()
    local isOpen = false
    for idx, pop in pairs(_PopList) do
        if (not MgrUI.IsInHideList(pop.Uid.Name) or MgrUI.IsShowState(pop.Uid)) then
            isOpen = true
        end
    end
    return isOpen
...
function MgrUI.IsPopOpenOutCou(_name,_list)
    ...
    for idx, pop in pairs(_PopList) do
        print(pop.Uid.Name)
        if (not MgrUI.IsInHideList(pop.Uid.Name) or MgrUI.IsShowState(pop.Uid)) and not MgrUI.IsInTable(pop.Uid.Name,_list) then
            if _name ~= pop.Uid.Name then
                isOpen = true
            end
        end
