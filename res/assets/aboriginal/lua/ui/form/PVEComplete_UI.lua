...
function M:OnShow(pData)
    ...
    local raidDmg = BlueprintViewModel.GetCurRaidDmg(self.pData.levelID);
    if nil ~= raidDmg then
        self.TextBlueprintDmg().gameObject:SetActive(true);
        self.TextBlueprintDmg().text = MgrLanguageData.GetLanguageByKey("ui_age_text77") .. " " .. raidDmg;
        self.TextBlueprintDmg2().gameObject:SetActive(true);
        self.TextBlueprintDmg2().text = MgrLanguageData.GetLanguageByKey("ui_age_text77") .. " " .. raidDmg;
    else
        self.TextBlueprintDmg().gameObject:SetActive(false);
        self.TextBlueprintDmg2().gameObject:SetActive(false);
    end
    ...
    if BattleRoleData.Bool_Pass == true then
        ...
        ---显示隐藏扫荡
        self.Btn_AutoBattle().gameObject:SetActive(pointData.star == 7 and pointData.battleMap ~= nil and pointData.battleMap ~= "0" and pointData.battleMap ~= "" and BattleRoleData.Bool_Pass and pointData:CheckGuide() == false)
        self.Btn_AutoBattle().gameObject:SetActive(BattleManager.CurActivityBossPointInfo ~= nil and BattleManager.CurActivityBossPointInfo.levelPerfectCount >= 5)
        ---扫荡点击事件
        UIEvent.LuaClick(self.Btn_AutoBattle().gameObject, function()
            if SysLockControl.CheckSysLock(2001) then   --是否解锁扫荡
            else
                MgrUI.Pop(UID.PopTip_UI,{string.format(SysLockControl.GetSystemLockTips(2001)),1},true)
                return
            end
            if pointData.star == 0 then     --是否完美通关
                return
            end
            if pointData.battleMap == nil or pointData.battleMap == "0" or pointData.battleMap == "" then
                return
            end

            MgrUI.Pop(UID.SweepPop_UI,{pointData},true)
        end)
    ...
    ---显示下一个剧情关
    --self.Btn_NextBattle().gameObject:SetActive(pointData.star ~= 0 and pointData.o_fronts ~= nil and pointData.o_fronts ~= "" and pointData.o_fronts ~= "0")
    ---下个区域点击事件
    --UIEvent.LuaClick(self.Btn_NextBattle().gameObject, function()
    --    ---如果是剧情活动关卡
    --    if pointData.type == 16 then
    --        for i, point in pairs(StormViewModel.CacheAllPointData) do
    --            local str = string.split(pointData.o_fronts,"_")
    --            if tonumber(str[1]) == point.id and tonumber(str[2]) == point.scrollID then
    --                ---如果当前索引+1超出最大索引上限
    --                local _index = EventRaidViewModel.CurIndex + 1 > tonumber(SteamLocalData.tab[117019][2]) and tonumber(SteamLocalData.tab[117019][2]) or EventRaidViewModel.CurIndex + 1
    --                ---当前活动关卡ID
    --                EventRaidViewModel.CurID = point.id
    --                ---去获取Boss血量数据
    --                EventRaidViewModel.GetBossData2(_index,function()
    --                    ---进入下一个剧情关卡
    --                    StormViewModel.OpenNextPlotUI(pointData)
    --                    MgrUI.ClosePop(self.Uid)
    --                end)
    --            end
    --        end
