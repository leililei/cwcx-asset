...
function PassportControl.GetAllPassportReward(awardId)
    ...
    for i,v in ipairs(MonthpassawardLocalData.tab) do
        if v[3] == awardId and v[7] == PassportInfo.version then
...
function PassportControl.BuyPassportLevelACK(buffer,tag)
    ...
    if tab.errNo~=0 then
        ItemControl.AckError = true
        if tab.errNo == ServerErrorCode.BuyPassesLevel_515 then
...
function PassportControl.Contains(target,table)
    ...
    for k,v in pairs(table) do
        if MonthpassawardLocalData.tab[tonumber(k)][2] == target and MonthpassawardLocalData.tab[tonumber(k)][7] == PassportInfo.version then
            return true
        end
    end
    return false
...
function PassportControl.ContainsSeniorReward(target,receivedReward)
    ...
    for k,v in pairs(receivedReward) do
        if MonthpassawardLocalData.tab[tonumber(k)][2] == target and MonthpassawardLocalData.tab[tonumber(k)][7] == PassportInfo.version and v == 2 then
            return true
        end
    end
    return false
...
function PassportControl.PassesRewardID(sortID)
    local allPassportReward = PassportControl.GetAllPassportReward(PassportInfo.activityId)
