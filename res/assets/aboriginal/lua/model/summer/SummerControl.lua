...
function SummerControl.GetPreheaData()
    ...
    for i, v in ipairs(PreheatList) do
        local tOpenTime = Global.GetTimeByStr(v.open_time)
        local tEndTime = Global.GetTimeByStr(v.end_time)
...
function SummerControl.SummerAwardREQ(callBack)
    local SummerAwardREQ = {
        rev = "0"
    }
    ---序列化
    local bytes = assert(pb.encode('PBClient.ClientSummerAwardREQ',SummerAwardREQ))

    ---发送数据
    MgrNet.SendReq(MID.CLIENT_SUMMER_AWARD_REQ,bytes,0,nil,function(buffer,tag)
        local info = assert(pb.decode('PBClient.ClientSummerAwardACK',buffer))
        ---查看table内容
        if info.errNo ~= 0 then
            
        end
    end,function(buffer,tag)
        local info = assert(pb.decode('PBClient.ClientSummerAwardNTF',buffer))
        CurAward = true
...
---预热红点
function SummerControl.CheckPreheatRedDot()
    RedDotControl.GetDotData("PreheatXiGua"):SetState(false)
    if not ActivityControl.CheckActiveOpen(ActivityControl.activityTypeEnum.PREHEAT) then
        return
    end
    RedDotControl.GetDotData("PreheatXiGua"):SetState(not CurAward)
end
------------------------夏活数据------------------------
...
function SummerControl.CheckChapterTimeOpen(_type,_scrollid)
    ...
    for i, chapterData in ipairs(tChapterData) do
        ...
        elseif chapterData.Chaptertype == nil then
            if Global.CheckOnTime(TimeControl.GetTimeTable(chapterData.chaptertime)) then
                isOpen = true
                break
            end
        end
