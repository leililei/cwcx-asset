...
function MgrSce.Load(sceName, cell)
    if MgrSce.StartClear then
        return
    end
    MgrSce.StartClear = true
...
function MgrSce.ClearBack(call)
    ...
    MgrTimer.AddDelayNoName(0.1, function ()
        ...
        MgrUI.CloseAllPop()
        MgrSound.ClearBGMSheet()
        MgrRes.NowClearImmediate()
        call()
    end)
end

function MgrSce.SetClearSceneImagePath(path)
    CMgrSce:SetClearSceneImagePath(path)
end
