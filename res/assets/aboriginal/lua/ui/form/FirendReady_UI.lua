...
function M:InitButton()
    ...
    UIEvent.LuaClick(self.Btn_Return().gameObject,Handle(self, function()
        if self.isChange then
            MgrUI.Pop(UID.ConfirmPop_UI,{MgrLanguageData.GetLanguageByKey("pvpready_ui_tips3"),Handle(self,self.SaveFightTeam),nil,2},true)
        else
...
function M:SaveFightTeam()
    ...
    ---保存战斗阵型
    TeamControl.ChangeTeamInfo(10004,fighters)
    TeamControl.SendSaveTeamData({10004},true,Handle(self,function()
...
function M:LoadTeamRole(teamIndex,saveTeam)
    ...
    if #team.info == 0 and (teamIndex == 10000 or teamIndex == 10001 or teamIndex == 10002 or teamIndex == 10004) then  ---如果队伍为空
    ...
    if saveTeam == false then
    else
        TeamControl.ChangeTeamInfo(10004,newTeam.info)
        TeamControl.SendSaveTeamData({10004},false)
    end
