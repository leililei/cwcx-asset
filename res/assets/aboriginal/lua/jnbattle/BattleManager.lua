...
require("LocalData/CalltypeLocalData")
...
function BattleManager.StartBattle(Uiupdata,BuildNoSpine,standType,BanSkill)  ---BuildNoSpine == 1 时不创建spine
    ...
    then
        ...
        if (BattleManager.FightType == BattleManager.MonsterStandType.MONSTER
                ...
                or BattleManager.FightType == BattleManager.MonsterStandType.BOSS_MONSTER
        ...
        elseif CJNBattleMgr.Instance.worldBossBattle and BattleManager.FightType > BattleManager.MonsterStandType.MONSTER     ---联合讨伐
        then
            ---设置联合讨伐右侧怪物参数
            local bossData =EventRaidControl.GetLIANHETAOFAData().BossData[BattleManager.CurBossLevel]
            BattleManager.IdRight=bossData:GetBattleAllMonster("Id",1)
            BattleManager.SkinIdRight = {}
            BattleManager.Right_Lv = bossData:GetBattleAllMonster("Level",1)
            BattleManager.Right_StarLv = bossData:GetBattleAllMonster("Star",1)
            BattleManager.Right_Awake = bossData:GetBattleAllMonster("Awaken",1)
            BattleManager.Right_SkillLv = bossData:GetBattleAllMonster("SkillLv",1)
            BattleManager.Right_Pos = bossData:GetBattleAllMonster("SIndex",1)
            BattleManager.Right_Qoom = bossData:GetBattleAllMonster("Scale",1)
            BattleManager.Right_IsBoss = bossData:GetBattleAllMonster("IsBoss",1)
            BattleManager.RightFW1_ID = bossData:GetBattleAllMonster("Core1",1)
            BattleManager.RightFW1_ZB = bossData:GetBattleAllMonster("CoreProperties1",1)
            BattleManager.RightFW1_Skill = bossData:GetBattleAllMonster("CoreSkill1",1)
            BattleManager.RightFW2_ID = bossData:GetBattleAllMonster("Core2",1)
            BattleManager.RightFW2_ZB = bossData:GetBattleAllMonster("CoreProperties2",1)
            BattleManager.RightFW2_Skill = bossData:GetBattleAllMonster("CoreSkill2",1)
            ---若存在二次战斗的怪物数据，分类整理好等待调用
            BattleManager.IdRight2 = bossData:GetBattleAllMonster("Id",2)
            BattleManager.SkinIdRight2 = {}
            BattleManager.Right_Lv2 = bossData:GetBattleAllMonster("Level",2)
            BattleManager.Right_StarLv2 = bossData:GetBattleAllMonster("Star",2)
            BattleManager.Right_Awake2 = bossData:GetBattleAllMonster("Awaken",2)
            BattleManager.Right_SkillLv2 = bossData:GetBattleAllMonster("SkillLv",2)
            BattleManager.Right_Pos2 = bossData:GetBattleAllMonster("SIndex",2)
            BattleManager.Right_Qoom2 = bossData:GetBattleAllMonster("Scale",2)
            BattleManager.Right_IsBoss2 = bossData:GetBattleAllMonster("IsBoss",2)
            BattleManager.RightFW1_ID2 = bossData:GetBattleAllMonster("Core1",2)
            BattleManager.RightFW1_ZB2 = bossData:GetBattleAllMonster("CoreProperties1",2)
            BattleManager.RightFW1_Skill2 = bossData:GetBattleAllMonster("CoreSkill1",2)
            BattleManager.RightFW2_ID2 = bossData:GetBattleAllMonster("Core2",2)
            BattleManager.RightFW2_ZB2 = bossData:GetBattleAllMonster("CoreProperties2",2)
            BattleManager.RightFW2_Skill2 = bossData:GetBattleAllMonster("CoreSkill2",2)
        ...
        then    --玩家对玩家
            ...
            BattleManager.SEquipRight=PVPViewModel.GetAllRoleSEquip(right)
            ...
            BattleManager.RightAllFlags = PVPViewModel.GetAllFlags(right)
            ---设置PVP战斗左侧角色参数
            BattleManager.IdLeft=PVPViewModel.GetAllRoleId(Left)
            BattleManager.SEquipLeft=PVPViewModel.GetAllRoleSEquip(Left)
            ...
            BattleManager.LeftAllFlags = PVPViewModel.GetAllFlags(Left)
        end
    end
    ...
    BattleManager.SetDelGameID(#BattleManager.AllRole+1)
end
...
function BattleManager.StartSecondPhaseBattle(BanSkill)
    ...
    for i,v in pairs(BattleManager.RightTeam) do
        if tonumber(v.ID) == 600002 or tonumber(v.ID) == 900002 or tonumber(v.ID) == 600009 or tonumber(v.ID) == 900009 then
            --隐藏耶梦加得遮挡相机的身体
        ...
        then
            if PVPViewModel.PlayerIsAtk then
                ...
                if BattleManager.IsTest == nil then     --如果是回放的时候
                    ...
                    if BattleManager.SEquipLeft[leftAtkO] ~= nil then   --有特殊装备就不算原来的共鸣装备属性
                        local specialEquip = BattleManager.SEquipLeft[leftAtkO]
                        if specialEquip ~= nil then
                            tempRoleR.specialEquip = specialEquip
                            ReadData.InitRoleGear(tempRoleR,specialEquip:GetCurrentAttr(),true)
                            ...
                            for _,str in pairs(advanceSkillStr) do
                                ReadData.ReplaceSkill(str,tempRoleR,skillIndex)
                            end
                        end
                    else
                        if #BattleManager.LeftEquipLevel[leftAtkO] ~= 0 then
                            ---己方进阶共鸣装备
                            --BattleManager.SEquipLeft
                            ---获取角色共鸣装备1
                ...
                for i, fighter in pairs(BattleViewModel.Fighters) do
                    if fighter.pos == BattleManager.Left_Pos[leftAtkO] and fighter.isLeft then
                        tempRoleR.HP = fighter.hp;
                        tempRoleR.HPmax = fighter.maxHp;
                        --print("XPXP Left1. team: " .. leftAtkO .. ", id: " .. BattleManager.IdLeft[leftAtkO] .. ", pos: " .. BattleManager.Left_Pos[leftAtkO] .. ", figher: " .. i .. ", hp: " .. fighter.maxHp);
                        break;
                    end
                end
            else
                ...
                tempRoleR= ReadData.CreatRole(BattleManager.IdLeft[leftAtkO],BattleManager.SkinIdLeft[leftAtkO],BattleManager.Left_Lv[leftAtkO],BattleManager.Left_StarLv[leftAtkO],BattleManager.Left_SkillLv[leftAtkO],BattleManager.Left_Awake[leftAtkO],BattleManager.Left_Favor[leftAtkO],true,lvUp,BattleManager.LeftAllFlags[leftAtkO])
                ...
                for i, fighter in pairs(BattleViewModel.Fighters) do
                    if fighter.pos == BattleManager.Left_Pos[leftAtkO] and fighter.isLeft then
                        tempRoleR.HP = fighter.hp;
                        tempRoleR.HPmax = fighter.maxHp;
                        --print("XPXP Left2. team: " .. leftAtkO .. ",  id: " .. BattleManager.IdLeft[leftAtkO] .. ", pos: " .. BattleManager.Left_Pos[leftAtkO] .. ", figher: " .. i .. ", hp: " .. fighter.maxHp);
                        break;
                    end
                end
            end
            ...
            --添加音效
            local tPathName = BattleManager.SkinIdLeft[leftAtkO]==nil and BattleManager.IdLeft[leftAtkO] or BattleManager.SkinIdLeft[leftAtkO]
            MgrSound.AddCue("Audio/role/"..tPathName..".acb")
        end
        --id计数器增加
...
function BattleManager.initRight()
    if  BattleManager.FightType == BattleManager.MonsterStandType.BOSS or BattleManager.FightType == BattleManager.MonsterStandType.BOSS_BOSS or CJNBattleMgr.Instance.worldBossBattle then
        CJNBattleLoop.Instance:SetBossstate(2)
    else
        CJNBattleLoop.Instance:SetBossstate(0)
    end
    local rightRealAtkOrder = 1
    ---如果是联合讨伐模式，右边只创建一个并且从表中读取怪物配置数据
    if CJNBattleMgr.Instance.worldBossBattle then
        ---@type BattleRole
        for i = 1, #BattleManager.IdRight do
            if BattleManager.IdRight[i] == nil then
                return
            end

            local tempRoleR = ReadData.CreatMonster(BattleManager.IdRight[i],BattleManager.Right_Lv[i],BattleManager.Right_StarLv[i],BattleManager.Right_SkillLv[i],BattleManager.Right_Awake[i] ,tonumber(BattleManager.Right_Qoom[i]),rightRealAtkOrder)
            if BattleManager.IdRight[i]==900000  then
                BattleManager.BodySize=WorldBossData.Niai_BossSize
            elseif BattleManager.IdRight[i]==900001 then
                BattleManager.BodySize=WorldBossData.Suobeike_BossSize
            elseif BattleManager.IdRight[i]==900002 then
                BattleManager.BodySize=WorldBossData.Yemengjiade_BossSize
            end
            --添加音效
            MgrSound.AddCue("Audio/role/"..BattleManager.IdRight[i]..".acb")
            --id计数器增加
            BattleManager.GameIdCout= BattleManager.GameIdCout + 1
            --赋值
            tempRoleR.GameID = BattleManager.GameIdCout
            ...
            --记录自己的坐标
            local tempNum = BattleManager.Right_Pos[i]
            tempNum = tempNum - 1
            ...
            rightRealAtkOrder = rightRealAtkOrder + 1
        end
        return
    end
    ...
    if StormViewModel.CurPointData ~= nil and StormViewModel.CurPointData.pointType == 4 then
        local enemyHpTab = YueTaViewModel.GetEnemyHp()
        if enemyHpTab then
            for i,v in pairs(enemyHpTab) do
                if v.hP == 0 then
                    table.insert(removeTab,v.heroID)
                end
            end
        end
    end
    ...
    for i = rightAtkO, #BattleManager.IdRight do
        ...
        then
            ...
            then
                ...
                tempRoleR= ReadData.CreatRole(BattleManager.IdRight[rightAtkO],BattleManager.SkinIdRight[rightAtkO],BattleManager.Right_Lv[rightAtkO],BattleManager.Right_StarLv[rightAtkO],BattleManager.Right_SkillLv[rightAtkO],BattleManager.Right_Awake[rightAtkO],BattleManager.Right_Favor[rightAtkO],true,lvUp,BattleManager.RightAllFlags[rightAtkO])
                if BattleManager.IsTest==nil then
                    ReadData.SetMabt(tempRoleR)
                    ---核心属性
                    ...
                    ---共鸣装备属性
                    local role = HeroControl.GetRoleDataByID(tonumber(tempRoleR.ID))
                    if BattleManager.SEquipRight[rightAtkO] ~= nil then   --有特殊装备就不算原来的共鸣装备属性
                        local specialEquip = BattleManager.SEquipRight[rightAtkO]
                        if specialEquip ~= nil then
                            tempRoleR.specialEquip = specialEquip
                            ReadData.InitRoleGear(tempRoleR,specialEquip:GetCurrentAttr(),true)
                            local skillIndex = specialEquip:GetAdvanceSkillNum()
                ...
                tempRoleR.PVPEnemy = true
                for i, fighter in pairs(BattleViewModel.Fighters) do
                    if fighter.pos == BattleManager.Right_Pos[rightAtkO] and not fighter.isLeft then
                        tempRoleR.HP = fighter.hp;
                        tempRoleR.HPmax = fighter.maxHp;
                        --print("XPXP Right1. team: " .. rightAtkO .. ",  id: " .. BattleManager.IdRight[rightAtkO] .. ", pos: " .. BattleManager.Right_Pos[rightAtkO] .. ", figher: " .. i .. ", hp: " .. fighter.maxHp);
                        break;
                    end
                end
                ...
                for i, fighter in pairs(BattleViewModel.Fighters) do
                    if fighter.pos == BattleManager.Right_Pos[rightAtkO] and not fighter.isLeft then
                        tempRoleR.HP = fighter.hp;
                        tempRoleR.HPmax = fighter.maxHp;
                        --print("XPXP Right2. team: " .. rightAtkO .. ", id: " .. BattleManager.IdRight[rightAtkO] .. ", pos: " .. BattleManager.Right_Pos[rightAtkO] .. ", figher: " .. i .. ", hp: " .. fighter.maxHp);
                        break;
                    end
                end
            end
            ...
            tempPos_Y =(tempNum%3)+1
            --添加音效
            local tPathName = BattleManager.SkinIdRight[rightAtkO]==nil and BattleManager.IdRight[rightAtkO] or BattleManager.SkinIdRight[rightAtkO]
            MgrSound.AddCue("Audio/role/"..tPathName..".acb")
        else    ---非PVP战斗/非联合讨伐
            tempRoleR= ReadData.CreatMonster(BattleManager.IdRight[rightAtkO],BattleManager.Right_Lv[rightAtkO],BattleManager.Right_StarLv[rightAtkO],BattleManager.Right_SkillLv[rightAtkO],BattleManager.Right_Awake[rightAtkO],tonumber(BattleManager.Right_Qoom[rightAtkO]) ,rightRealAtkOrder)
            if tempRoleR.Occupation == 5 or tempRoleR.Occupation == 6 then
        ...
        if StormViewModel.CurPointData ~= nil and StormViewModel.CurPointData.pointType == 4 then  ---月塔关卡残血记录
            local lit = YueTaViewModel.GetEnemyHp()
            if lit then
                for i,v in pairs(lit) do
                    if v.heroID == tempRoleR.GameID then
                        tempRoleR.HP = tempRoleR.HP * v.hP/10000
                        if tempRoleR.HP > 0 and tempRoleR.HP < 1 then
                            tempRoleR.HP = 1
                        end
                    end
                end
        ...
        if next(removeTab) then
            for i,v in pairs(removeTab) do
                if tempRoleR.GameID == v then
                    tempRoleR.Remove = true
                    tempRoleR.AtkOrder = -1     --开战前就已被移除
                    break
                end
            end
        end
        if not tempRoleR.Remove then
            rightRealAtkOrder = rightRealAtkOrder + 1
        end
    ...
    for k, v in pairs(BattleManager.RightTeam) do
        if v ~= nil then
            ...
            then
                ...
                if BattleManager.IdRight[1] == 900002 or BattleManager.IdRight[1] == 600002 or BattleManager.IdRight[1] == 600009 or BattleManager.IdRight[1] == 900009 then
                    go.localScale = Vector3(1,1,1)
                    ---耶梦加德水花
            ...
            then
                ...
                if 10000 <= tonumber(tempRole.ID) and tonumber(tempRole.ID) < 20000 and 
                        (BattleManager.GameMode == BattleManager.GameModeType.RedTower or 
                                BattleManager.GameMode == BattleManager.GameModeType.PVP or 
                                BattleManager.GameMode == BattleManager.GameModeType.FriendPVP or 
            ...
            if tempRole.Remove then
                tempRole.myAni.gameObject:SetActive(false)
            end
        end
    end
end

function BattleManager.Summon(id,isLeft,pos,GameID,atkOrder,SummonReleaser)
    BattleManager.InitSingle(CalltypeLocalData.tab[id].monster,pos,GameID,atkOrder,isLeft,SummonReleaser)  --单个怪物数据
    BattleManager.InitSingleSpine(GameID,isLeft)
end

function BattleManager.InitSingle(roleInfo,pos,GameID,atkOrder,isLeft,SummonReleaser)
    local monsterInfo = string.split(roleInfo,"_")
    local tempRole = ReadData.CreatMonster_SummonObj(tonumber(monsterInfo[1]),tonumber(monsterInfo[3]),tonumber(monsterInfo[2]),tonumber(monsterInfo[5]),tonumber(monsterInfo[4]),tonumber(monsterInfo[7]),tonumber(monsterInfo[9]),tonumber(monsterInfo[10]),tonumber(monsterInfo[11]),tonumber(monsterInfo[12]),tonumber(monsterInfo[13]),tonumber(monsterInfo[14]),atkOrder)
    local tempNum = pos
    tempNum=tempNum-1
    local tempPos_X =math.floor(tempNum/3)+1
    local tempPos_Y =(tempNum%3)+1
    tempRole.PosX = tempPos_X
    tempRole.PosY = tempPos_Y
    tempRole.GameID = GameID
    tempRole.IsLeft = isLeft
    tempRole.isSummonObj = true     ---是召唤物
    if monsterInfo[9]  ~= "0" then
        local AbtArr = ReadData.GetGearAttr(monsterInfo[9],monsterInfo[10])
        ReadData.InitRoleGear(tempRole,AbtArr,true)
    end
    if monsterInfo[12]  ~= "0" then
        local AbtArr = ReadData.GetGearAttr(monsterInfo[12],monsterInfo[13])
        ReadData.InitRoleGear(tempRole,AbtArr,true)
    end
    ReadData.AddSkillId(tempRole,monsterInfo[11] ,monsterInfo[14])
    BattleManager.AllRole[tempRole.GameID] = tempRole
    ---写入队伍 索引为id
    if isLeft then
        ---遍历所在队伍，所有攻击顺序在召唤物之后的都+1
        for i,v in pairs(BattleManager.LeftTeam) do
            if v.AtkOrder >= tempRole.AtkOrder then
                v.AtkOrder = v.AtkOrder + 1
            end
        end
        BattleManager.LeftTeam[tempRole.GameID] = tempRole
    else
        ---遍历所在队伍，所有攻击顺序在召唤物之后的都+1
        for i,v in pairs(BattleManager.RightTeam) do
            if v.AtkOrder >= tempRole.AtkOrder then
        ...
        BattleManager.RightTeam[tempRole.GameID] = tempRole
    end
    BattleManager.Rank()
end

function BattleManager.InitSingleSpine(GameID,isLeft)
    local tempRole = BattleManager.AllRole[GameID]
    tempRole:RoleAniLoad(tempRole,0.2)
    print("BattleRole.CreartData 2")
    BattleRole.CreartData(tempRole)
    ---统合所有特效ID
    local allEffectID = {}
    for i = 1, #tempRole.AtkEffectId do
        table.insert(allEffectID,tonumber(tempRole.AtkEffectId[i][1][1]))
    end
    for i = 1, #tempRole.DeathEffectId do
        table.insert(allEffectID,tonumber(tempRole.DeathEffectId[i][1][1]))
    end
    for i = 1, #tempRole.VertigoEffectId do
        table.insert(allEffectID,tonumber(tempRole.VertigoEffectId[i][1][1]))
    end
    for i = 1, #tempRole.HitEffectId do
        table.insert(allEffectID,tonumber(tempRole.HitEffectId[i][1][1]))
    end
    for i = 1, #tempRole.ZJEffectId do
        table.insert(allEffectID,tonumber(tempRole.ZJEffectId[i][1][1]))
    end
    ---创建特效
    JNTurnEffectMgr.LuaCeratAll(allEffectID,#allEffectID)
    tempRole.myAni:PlayAni(1)
    if isLeft then
        --给地板赋值    位置、id和职业
        if not tempRole.Remove then
            tempRole.myAni:XYSetPos2(tempRole.PosX, tempRole.PosY,tempRole.GameID,tempRole.Occupation)
        end
        tempRole.myAni:SetNpc_Reverse() --怪物要反转
    else
        --给地板赋值    位置、id和职业
        if not tempRole.Remove then
            tempRole.myAni:XYSetPos_Right(tempRole.PosX, tempRole.PosY,tempRole.GameID,tempRole.Occupation)
    ...
    ---登场特效
    BattleRole.CreatEffFollowAni(tempRole,true,0)
    BattleRole.SetFlyIn(tempRole,0)
    ---隐藏原有尸体
    if isLeft then
    else
        for i,v in pairs(BattleManager.RightTeam) do
            if v.Remove then
                ---死亡角色尸体隐藏
                local body = CJNBattleMgr.Instance:IdToAniObj_All(v.GameID)
                body.transform.gameObject:SetActive(false)
            end
        end
    end
    tempRole.myAni.roleData:GameStart()     ---初始化显示血量、阴影、更新特效
end
...
function BattleManager.GameStart(TrueOrFalse,closePanelCallBack)
    ...
    if closePanelCallBack then
        MgrTimer.AddDelayNoName(0,BattleViewModel.StartBattle(BattleManager.GameMode,closePanelCallBack))
    else
        BattleManager.GameStart_LuatoC(function ()
            MgrTimer.AddDelayNoName(0,BattleViewModel.StartBattle(BattleManager.GameMode))
        end)
    end
...
BattleManager.LeftAtkOrderCout=1
function BattleManager.CreartRoleLeft(GameDataID,skinID, LV, StartLV, SkillLV, IsAwken, roleType, userID, friend, favor,equipUnlockLV,heroFlag,heroCurSkill)
    --读出RoleattributeLocalData表中所有数据
    local tempRoleL= ReadData.CreatRole(GameDataID,skinID,LV,StartLV,SkillLV,IsAwken,favor,nil,equipUnlockLV,heroFlag,heroCurSkill)
    ...
    --技能等级设置
    if heroFlag then
        tempRoleL.heroFlag = heroFlag
    end 
    if heroCurSkill then
        tempRoleL.heroCurSkill = heroCurSkill
    end
...
function BattleManager.SetOrderAfter(dragRole)
    ...
    for i, role in pairs(BattleManager.LeftTeam) do
        ...
        if role.GameID ~= nil then
            if role.GameID > dragRole.GameID then
                BattleManager.AllRole[role.GameID-1] = BattleManager.AllRole[role.GameID]
                BattleManager.AllRole[role.GameID] = nil
    ...
    --设置上一个被删除的GameID
    BattleManager.SetDelGameID(dragRole.GameID)
end
--开始战斗的摄像机模式
...
function BattleManager.FAndShowRound(_temprole,_posx ,_posy)
    ...
    if tempAtkRole.SleRole==nil or tempAtkRole.SleRole.myAni.gameObject==nil then
        ---设置地板颜色   新增职业类型11
        BattleManager.CjnMgr:SetFloorStata( _temprole.IsLeft,temprolex,temproley,20+_temprole.Occupation ,-1)
        return
    end
    local IsRed
    ...
    table.insert(tabOcc,20+_temprole.Occupation)
...
function BattleManager.ReturnToMainScene(_win)
    ...
    then
        ...
        MgrUI.Pop(UID.WinOrFail_UI,{BattleRoleData.Bool_Pass,function()
            Event.Go("PanelFight")
            ---战术指导结算
            if BattleViewModel.GuideTab == nil or BattleViewModel.isLeftWin == false then
                MgrBattle.CloseFight()
            else
                ---显示奖励弹窗
    ...
    then
        if StormViewModel.IsAnaWorldBoss then
            ...
            MgrUI.Pop(UID.WinOrFail_UI,{BattleRoleData.Bool_Pass,function()
                ...
                MgrUI.Pop(UID.WBComplete_UI,{},true)
            end},true)
        else
            ---非模拟战打开胜利/失败特效和结算弹窗
            MgrUI.Pop(UID.WinOrFail_UI,{BattleRoleData.Bool_Pass,function()
    ...
    elseif BattleManager.GameMode == BattleManager.GameModeType.BuDoukaiPVP then
        ...
        MgrUI.Pop(UID.WinOrFail_UI,{BattleRoleData.Bool_Pass,function()
            ...
            Event.Go("JumpOutClose")
            Event.Go("PanelFight")
            if not PVPViewModel.CheckBuDoukaiPvpData() then
                ---打开武道会PVP结算界面
...
--所有技能结算完毕,回合结束
function BattleManager.ClearLuaData(callback)
...
---判断战斗有无Boss
function BattleManager.HaveBoss()
    for i,v in pairs(BattleManager.AllRole) do
...
---设置核心类型(0进攻 1防守)
function BattleManager.SetCoreType(_armorType)
    ReadData.SetCoreType(_armorType)
end
---设置上一个被删除的GameID
function BattleManager.SetDelGameID(_gameID)
    DelLeftGameId = _gameID
end
function BattleManager.GetDelGameID()
    return DelLeftGameId
end
