...
function PlotViewModel.OpenPlotUI(plot,callback,isGoClose,isNotAutoBack,isNovice)
    ...
    MgrSce.SetClearSceneImagePath(SettingViewModel.ReturnUnlockRandomImage().img)
...
require("LocalData/StoryreplayLocalData")
PlotViewModel.ReviewActivityId = nil;
PlotViewModel.ReviewCheckPointId = nil;
function PlotViewModel.GetStoryReplayInfo(id)
    for _, v in pairs(StoryreplayLocalData.tab) do
...
function PlotViewModel.CheckReviewRedDot()
    for _, replayInfo in pairs(StoryreplayLocalData.tab) do
        -- 章节
        local chapterInfo = ChapterLocalData.tab[tonumber(replayInfo.chapterid)];
        if nil ~= chapterInfo then
            local checkpoints = string.split(chapterInfo.simple,",");
            for _, checkpointId in pairs(checkpoints) do
                -- 关卡
                local isUnlock = StormControl.CheckPointLock(tonumber(checkpointId));
                if isUnlock then
                    -- 解锁
                    local pointData = StormControl.GetStormPointByID(tonumber(checkpointId));
                    if pointData.star > 0 then
                    else
                        -- 没打
                        RedDotControl.GetDotData("PlotReview"):SetState(true);
                        return;
                    end
                end
            end
    ...
    RedDotControl.GetDotData("PlotReview"):SetState(false);
end
function PlotViewModel.CloseReviewRedDot()
    RedDotControl.GetDotData("PlotReview"):SetState(false);
end
