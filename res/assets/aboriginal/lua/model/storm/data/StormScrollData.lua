...
function StormScrollData:PushConfig(config)
    ...
    self.thumbnail_map = config.thumbnail_map;
end
---添加挑战红巨配置属性
...
---添加月塔配置属性
function StormScrollData:PushMonthTower(points)
    self.id = 1
    local pointStr = string.split(points,",")
    for idx, idStr in pairs(pointStr) do
        self.points[#self.points + 1] = tonumber(idStr)
    end
    self.name = MgrLanguageData.GetLanguageByKey("月塔")
    self.type2 = 22
    self.bg = "Storm/ScrollBg_20_22"  ---卷背景
end
---添加挑战战术指导配置
...
function StormScrollData:GetMaxBoxStar()
    ...
    for i, id in pairs(self.points) do
        ...
        if data:CheckIsBattle() and not data:CheckGuide() then
