...
MgrNet.ResendMID = {                    -- 需要重发的协议ID
    ...
    MID.CLIENT_GET_MONTHLY_TOWER_REQ,
}
...
function MgrNet.GetCurServerVer()
    return MgrNet.CS:GetCurServerVer()
end
...
function MgrNet.SendReq(msgId, buffer, tag, ReqFunc, ACKFunc, NTFFunc)
    ...
    if ACKFunc then
        MessageEvent.Add(msgId + 1,function(...)
            MgrNet.ReceiveACKLog(msgId,...)
            if MID.CLIENT_TASK_REWARD_ONE_KEY_REQ == msgId or MID.CLIENT_GET_LOTTERY_LOG_REQ == msgId then
                ACKFunc(...)
                return
            end
            ---服务器错误提示
            local ErrBack,tag = MgrNet.ACKErroTips(msgId, ...)
            ACKFunc(ErrBack,tag)
        end)
        ---记录消息占用
...
---服务器错误提示
function MgrNet.ACKErroTips(msgId, buffer, tag)
    local tab = assert(pb.decode('PBClient.ClientVerifyACK',buffer))
    
    ---排除需要错误码的协议ID
    if MID.CLIENT_VERIFY_REQ == msgId or
       MID.CLIENT_RECONNECT_REQ == msgId or
       MID.CLIENT_BUY_PASSES_LEVEL_REQ == msgId or
       MID.CLIENT_N_GAME_PHASE_UP_REQ == msgId or
       MID.CLIENT_ACTIVITY_GET_REWARD_REQ == msgId or
       MID.CLIENT_EMAIL_DATA_REQ == msgId or
       MID.CLIENT_SAVE_PROGRESS_REQ == msgId or
       MID.CLIENT_TASK_PERFORM_REQ == msgId or
       MID.CLIENT_TEAM_SAVE_REQ == msgId or
       MID.CLIENT_ARMOR_MAKE_REQ == msgId or
       MID.CLIENT_RECRUIT_REQ == msgId or
       MID.CLIENT_EQUIP_SUPPLY_REQ == msgId or
       MID.CLIENT_RECRUIT_REPEAT_REQ == msgId or
       MID.CLIENT_NEW_RECRUIT_REQ == msgId or
       MID.CLIENT_HIGH_LADDER_BATTLE_EX_REQ == msgId or
       MID.CLIENT_BUY_BLADDER_REQ == msgId or
       MID.CLIENT_LIMIT_SIGN_REQ == msgId or
       MID.CLIENT_FRIEND_APPLY_REQ == msgId or
       MID.CLIENT_SET_LEVEL_STAR_REQ == msgId or
       MID.CLIENT_DECOMPOSE_REQ == msgId or
       MID.CLIENT_CHANGE_LOCK_REQ == msgId or
       MID.CLIENT_SET_LEVEL_STAR_EX_REQ == msgId or
       MID.CLIENT_BOSS_BATTLE_E_REQ == msgId or
       MID.CLIENT_TOWER_BATTLE_REQ == msgId or
       MID.CLIENT_GUIDE_BATTLE_REQ == msgId or
       MID.CLIENT_SET_EVENT_BOSS_LEVEL_STAR_REQ == msgId or
       MID.CLIENT_MONTHLY_TOWER_FIGHT_REQ == msgId or
       MID.CLIENT_CHOOSE_BATTLE_REWARD_REQ == msgId or
       MID.CLIENT_CHOOSE_LADDER_BATTLE_REWARD_REQ == msgId or
       MID.CLIENT_TOWER_REWARD_REQ == msgId or
       MID.CLIENT_GUIDE_REWARD_REQ == msgId or
       MID.CLIENT_GET_HIGH_LADDER_RANK_REQ == msgId or
       MID.CLIENT_BUY_GOODS_REQ == msgId or
       MID.CLIENT_BUY_RAND_SHOP_ITEM_REQ == msgId or
       MID.CLIENT_GET_BUY_NUMBER_REQ == msgId or
       MID.CLIENT_SIGN_IN_REQ == msgId or 
       MID.CLIENT_SELECT_UP_GACHA_REQ == msgId or 
       MID.CLIENT_GET_CHAMPION_MATCH_REQ == msgId or 
       MID.CLIENT_EXPAND_REQ == msgId
    then
    else
        if tab.errNo ~= 0 then
    ...
    local tab1 = assert(pb.encode('PBClient.ClientVerifyACK',tab))
    return tab1,tag
end

--- 发现许多请求都是使用逻辑相似的 ACK 方法
--- 又因 SendReq ACK 逻辑不便修改，特别添加该通用 ACK
---@param pbKey string
---@param buffer string
---@param tag any?
---@return int
function MgrNet.CommonACK(pbKey, buffer, tag)
    local tab = assert(pb.decode(pbKey, buffer))
    local errNo = tab.errNo or -1
    print(string.format("%s.errNo:%s", pbKey, errNo))
    if errNo ~= 0 then
        Global.PopTip("Error" .. errNo, 2, true)
    end
    return errNo
end
...
function MgrNet.VerifyREQReceive(recCell,ackCell,ntfCell)
    ...
    ---服务器额外发送一条记录活动限时签到的NTF
    MgrNet.RegisterNTF(MID.CLIENT_LIMIT_SIGN2_INFO_NTF, function(buffer,tag)
        local tab = assert(pb.decode('PBClient.ClientLimitSign2InfoNtf',buffer))
        if tab then
            require("Model/LimitSign/LimitSign_NewControl")         ---新限时签到活动
            LimitSign_NewControl.Init()
    ...
    ---服务器额外发送一条记录活动限时签到的NTF
    MgrNet.RegisterNTF(MID.CLIENT_LIMIT_SIGN2_LOGIN_NTF, function(buffer,tag)
        local tab = assert(pb.decode('PBClient.ClientLimitSign2LoginNtf',buffer))
        if tab then
            ActivityControl.SetLimitSignData(tab.info)
        end
    end)
    ---服务器额外发送一条自选卡池的NTF
    MgrNet.RegisterNTF(MID.CLIENT_SELECT_UP_NTF, function(buffer,tag)
        local tab = assert(pb.decode('PBClient.ClientSelectUpNtf',buffer))
        if tab then
            CardDrawControl.InitSelectPoolList(tab.lotteryInfo)
        end
    end)

    --新手活动时间更新
    MgrNet.RegisterNTF(MID.CLIENT_OPEN_NEW_ACTIVITY_NTF, function(buffer,tag)
        local tab = assert(pb.decode('PBClient.ClientOpenNewActivityNtf',buffer))
        TimeControl.AddTimeTable(tab.newTimeData,tab.openActivity)
    end)
    ...
    if MgrNet.hasReConnect == false then
        ...
        if not MgrNet.ConnectServer(function(err,msgId)
            ...
            if info.errNo ~= 0 then
                ...
                if info.errNo == ServerErrorCode.Reconnect_203 then  ---被顶号
...
function MgrNet.HttpRegister(username,password,actNumber,func,type)
    ---注册的http路径
    local register_url = "&url/httpRegister"
...
function MgrNet.IosLogin(userId, token, func)
    ---登录的http路径
    local login_url = "&url/httpLogin"
    ...
    local from = {
        ...
        "userId="..userId,
        "accessToken="..token,
        "os="..MgrSdk.GetPlatform(),
        "device="..MgrSdk.GetSdkDeviceUDID(),
    }
...
function MgrNet.HttpChangeBind(userId, platform, openid, token, newToken, func)
    ---登录的http路径
    local login_url = "&url/changeBind"
...
function MgrNet.HttpGetOrderGenernal(userId,productId,callback)
    ---获取订单的http路径
    local get_url = "&url/GetOrderGeneral"
    ---配置表单
    local from = {
        "user_id="..userId,
        "product_id="..productId,
...
function MgrNet.ReqRechargeInfo()
    local rechargeTab = {}
    local rechargeBytes = assert(pb.encode('PBClient.ClientRechargeInfoREQ',rechargeTab))
    MgrNet.SendReq(MID.CLIENT_RECHARGE_INFO_REQ, rechargeBytes, 0, nil, function (buffer, tag)
        local tab = assert(pb.decode('PBClient.ClientRechargeInfoACK',buffer))
        print(tab.errNo)
        -- if tab.errNo~=0 then
        --     MgrUI.Pop(UID.PopTip_UI,{MgrLanguageData.GetLanguageByKey("activityCcontrol_tips1"),2},true)
        -- end
    end, function (buffer, tag)
        local info = assert(pb.decode('PBClient.ClientRechargeInfoNTF',buffer))
        print(serpent.block(info))
        ---弹出充值成功提示
        if MgrUI.GetCurUI().Uid == UID.Shop_UI or MgrUI.GetCurUI().Uid == UID.ShopHome_UI then
            for k,v in pairs(info.goods) do
                --数量对不上弹出弹窗
                if ItemControl.GetItemByIdAndType(v.goodsID,v.goodsType).count ~= v.goodsNum then
                    MgrUI.Pop(UID.PaySuccess_UI,{2},false)
                end
            end
        end
        ItemControl.PushGroupItemData(info.goods,ItemControl.PushEnum.cover)
        ActivityControl.UpdateRecharge(info.events)
        ShopControl.PushGroupShopItem(info.shops)
        ---是否存在大月卡时效
        if info.bigMonthCardBuyTime ~= nil and info.bigMonthCardVersion ~= nil then
            PassportControl.PushPassport(info.bigMonthCardVersion,info.bigMonthCardBuyTime)
            if info.bigMonthCardBuyTime ~= 0 and info.bigMonthCardVersion == ActivityLocalData.tab[20000][6] then
                PassportViewModel.SetPassesSeniorLock(true)
            end
            PlayerControl.GetPlayerData().bigMonthCardVersion = info.bigMonthCardVersion
            PlayerControl.GetPlayerData().bigMonthCardBuyTime = info.bigMonthCardBuyTime
        end
        ---是否存在小月卡时效
        if info.monthCardBuyTime ~= nil and info.monthCardRemaining ~= nil then
            PlayerControl.GetPlayerData().monthCardBuyTime = info.monthCardBuyTime
            PlayerControl.GetPlayerData().monthCardRemaining = info.monthCardRemaining
        end
