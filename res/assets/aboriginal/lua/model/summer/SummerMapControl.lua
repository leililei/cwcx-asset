...
---获取地图的偏移值
function SummerMapControl.GetDValue(_pos)
    local tStartPos = Vector3(SummerMapControl.StartPos.x,SummerMapControl.StartPos.y,0)
    return tStartPos+_pos
end
...
function SummerMapControl.LoadCurMapMark()    
    local tPoint = SummerMapControl.GetChapter()
...
---初始化各地图走过的格子数据
function SummerMapControl.InitMapMark(_activeData)
    local tEndKey = "SummerMapMarkEndTime"..PlayerControl.GetPlayerData().UID
    local tMapMarkEndTime = UnityEngine.PlayerPrefs.GetString(tEndKey)
    
    ---如果结束时间不同，清理本地走过的格子数据
    if tMapMarkEndTime ~= _activeData.endTime then
        UnityEngine.PlayerPrefs.SetString(tEndKey,_activeData.endTime)
        for i, v in pairs(_activeData.chapterId) do
            local tKey = "SummerMapMark"..PlayerControl.GetPlayerData().UID..v
            UnityEngine.PlayerPrefs.SetString(tKey,"")
        end
    end
end
---地图章节ID
function SummerMapControl.SetChapter(_chapter)
    ChapterID = _chapter
end
function SummerMapControl.GetChapter()
    return ChapterID
end
---地图章节数据
function SummerMapControl.SetChapterData(_chapterData)
    ChapterData = _chapterData
end
function SummerMapControl.GetChapterData()
    return ChapterData
end
---帮助数据
function SummerMapControl.SetHelp(_helpData)
    HelpData = _helpData
end
function SummerMapControl.GetHelp()
    return HelpData
end
