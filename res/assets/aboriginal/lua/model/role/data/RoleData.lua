...
function RoleData:ctor(id)
    self.id = id                    ---id
    self.armor1 = 0                 ---核心1id
    self.armor1Pro = 0              ---核心1占比
    self.armor1Skill = 0            ---核心1技能
    self.armor2 = 0                 ---核心2id
    self.armor2Pro = 0              ---核心2占比
    self.armor2Skill = 0            ---核心2技能
    self.core1 = nil
    self.core2 = nil
    self.type = 1                   ---类型1：玩家角色，类型2：Npc
    self.heroDefArmor1 = 0          ---核心1id(防御)
    self.heroDefArmor2 = 0          ---核心2id(防御)
    local config = nil
    if id > 199999 then
        ...
        self.iconCareer = "Attribute/ProIcon_"..config[3]   ---角色职业图标
        self.iconBattleFrame = "Quality/RoleRankN_"..self.rank   ---角色头像边框(BattleUI)
        self.iconTeamFrame = "Quality/RankF_"..self.rank   ---角色阵营头像边框
        self.atkDescription = config[26]                                    ---角色攻击简介
        self.label = config[84]                                             ---角色定位
        self.cgSpine = config[85]                                           ---CG切换 填0没有切换填1可以切换
        self.New_Career = tonumber(config[3])                                         ---暂时只用来判断觉醒消耗
        self.ExSkillID = config[41] ~= "0" and tonumber(string.split(config[41],'{')[1]) or 0   ---EX技能ID
        self.SpecialEquip = EquipControl.GetSingleSpecialEquip(self.id)     ---超限装备
        self.SpecialLevel = self.SpecialEquip and self.SpecialEquip.evolutionLevel or 0     ---超限装备等级
        self.SpecialEquipUnlock = self.SpecialEquip and self.SpecialEquip:CheckLockState() or false   ---超限装备解锁状态
        self.BlueprintGroup = config[88]                ---蓝图组
    end
    self.tryHeroUID = nil;
    self.tryHeroProps = {};
    self.sEquipOff = false                                      ---是否关闭超限装备
end
...
local EquipInfo = {
    ...
    heroCTime = 9,      ---获得时间
    heroFavor = 10,      ---好感度
    heroCurSkill = 11,  ---设置技能等级
    heroFlag = 12,      -- 是否设置过技能等级	0默认未设置 <<1 设置技能等级
    heroDefArmor1 = 13,      -- 符文id(防御)
    heroDefArmor2 = 14,      -- 符文id(防御)
    sEquipOff = 15,      -- 是否关闭超限装备
}
...
function RoleData:PushHeroData(hero)
    ...
    self.heroDefArmor1 = hero.heroDefArmor1
    self.heroDefArmor2 = hero.heroDefArmor2
    self.cTime = hero.heroCTime
    self.favor = hero.heroFavor
    self.heroFlag = hero.heroFlag
    self.heroCurSkill = hero.heroCurSkill
    self.lockState = true
    self.sEquipOff = hero.sEquipOff == 0
end
...
function RoleData:PushSingleHeroData(fighterAttr,skinId)
    ...
    self.heroFlag = fighterAttr.heroFlag
    ...
    if fighterAttr.heroFavor ~= nil then
        self.favor = fighterAttr.heroFavor
    end
    if fighterAttr.favor ~= nil then
        self.favor = fighterAttr.favor
    end
    if fighterAttr.SEquip ~= nil then
        self.SEquipID = fighterAttr.SEquip.ID
        self.SEquipLevel = fighterAttr.SEquip.Level
        self.SEquipGrade = fighterAttr.SEquip.Grade
    end
end
...
function RoleData:PushSingleFriendHeroData(fighterAttr)
    ...
    self.heroFlag = fighterAttr.heroFlag
    self.heroCurSkill = fighterAttr.heroCurSkill
    ...
    --设置技能等级
    if fighterAttr.heroFlag ~= nil and fighterAttr.heroFlag == true then
        self.ShowSkillLV = fighterAttr.heroCurSkill
    end
...
function RoleData:HeroAllEquipLevelMax()
    local maxLevel = tonumber(SteamLocalData.tab[105012][2])
    local isLevel = true
    local isLock = true
    ---好友角色属性计算
    if self.userID ~= nil then
        for i = 1, #self.equipLvArr do
            if self.equipLvArr[i] < maxLevel then
                isLevel = false
            end
        end
        return isLevel
...
function RoleData:GetHeroSkillLevel()
    if self.heroFlag ~= nil and self.heroFlag ~= false and self.heroFlag ~= 0 and self.heroCurSkill ~= nil and self.heroCurSkill < 10 then
        return self.heroCurSkill
    end
...
function RoleData:GetHeroShowSkillLv()
    --如果设置过技能等级直接返回设置后的技能等级
    if self.heroFlag ~= nil and self.heroFlag ~= 0 then
        return self.heroCurSkill
    end
    if self:CheckHeroEquipIsMax() then
        return self.skillLevel + 1
...
function RoleData:GetFriendHeroAttr()
    ...
    for i ,v in ipairs(self.equipLvArr) do
     ...
     if self.heroFlag ~= nil and self.heroCurSkill == nil then
         return ReadData.CreatRole(self.id,self.skin,self.level,self.star,self.skillLevel,self.awaken,self.favor,true,skillUp,self.heroFlag,self.skillLevel)     --好友技能等级传过来时是自身等级，没算装备提升
    else
        return ReadData.CreatRole(self.id,self.skin,self.level,self.star,self.skillLevel,self.awaken,self.favor,true,skillUp,self.heroFlag,self.heroCurSkill)     --好友技能等级传过来时是自身等级，没算装备提升
    end
end
function RoleData:GetHeroAttrWithCoreAndEquip()
    return ReadData.CreatRole(self.id,self.skin,self.level,self.star,self:GetHeroSkillLevel(),self.awaken,self.favor,nil,self:CheckHeroEquipIsMax(),self.heroFlag,self.heroCurSkill)
end
...
---@return CoreData 获取人物核心(槽位1，槽位2)(进攻0，防守1)
function RoleData:GetHeroCore(index,armorType)
    local id = self["armor"..index]
    if armorType == 1 then
        id = self["heroDefArmor"..index]
    end
    if id == nil or id == 0 then
        return nil
    end
...
---己方角色获取专属进阶装备
function RoleData:GetHeroSpecialEquip()
    return EquipControl.GetSingleSpecialEquip(self.id)
end
...
function RoleData:ReLoadHeroCore(type, coreId, armorType)
    if armorType == 0 then
        if type == 1 then
            self.armor1 = coreId
        elseif type == 2 then
        ...
        if type == 1 then
            self.heroDefArmor1 = coreId
        elseif type == 2 then
            self.heroDefArmor2 = coreId
        end
    end
end
...
function RoleData:SetTryHeroInfo(uID, props)
    self.tryHeroUID = uID;
    self.tryHeroProps = {};
    if nil == props then
        return;
    end
    self.tryHeroProps = props;
    for i, v in ipairs(props) do
        if v.field == PropEnum.Level then
            self.level = v.value
        elseif v.field == PropEnum.Rank then
            self.skillLevel = v.value
        elseif v.field == PropEnum.Star then
            self.star = v.value
        elseif v.field == PropEnum.Awaken then
            self.awaken = v.value == 1
        end
    end
end

function RoleData:SetSpecialEquipUnlock(_unlock)
    self.SpecialEquipUnlock = _unlock
end
---更新超限装备等级
function RoleData:RefreshSEquip(_level)
    self.SpecialLevel = _level
end
