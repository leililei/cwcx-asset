...
function ActiveChapterData:ctor(id)
    ...
    local timeC = TimeControl.GetTimeTable(config.chaptertime)
    if timeC then
        if timeC.timeType ~= 999 then
    ...
    self.walkMan = config.walkman == "0" and nil or string.split(config.walkman,',')                       ---走路小人形象
end
