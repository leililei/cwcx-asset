...
---家园远征任务数据(来自服务器)
ArkViewModel.sTaskData = {}
---当前远征任务
ArkViewModel.CurTaskID = nil
---当前远征唯一id
ArkViewModel.CurTaskGroupId = nil
---玩家英雄信息
...
--根据道具是否可以合成重新排序
function ArkViewModel.ResetEnough(list)
    local tab = {}
    local tab1 = {}
    for i, v in ipairs(list) do
        local enough = true
        
        local str = string.split(v.synthesisCost,',')
        local str1 = string.split(str[2],'_')
        local num = ItemControl.GetItemByID( tonumber(str1[2])).count
        local needNum = tonumber(str1[3])
        -- for k, j in ipairs(str) do
        --      str1 = string.split(j,'_')
        --     local num = ItemControl.GetItemByID( tonumber(str1[2])).count
        --     local needNum = tonumber(str1[3])
        --     if num <needNum and v.SynthesisCount1 == 0 then
        --         enough = false
        --     end
        -- end
        enough = v.SynthesisCount1 > 0 and num >= needNum
...
function ArkViewModel.GetSynthesisDataTypeTwo(list,sortIdx, isRise)
    ...
    Global.Sort(tab,sortGroup[sortIdx or 1],rise)
    Global.Sort(untab,sortGroup[sortIdx or 1],rise)
    tab =  ArkViewModel.ResetEnough(tab)
    for index, value in ipairs(untab) do
        table.insert(tab,value)
    end
    return tab
...
function ArkViewModel.CheckRoleIsSynthesis(role,roleTabDt)
    ...
    if roleData == nil then
        print("roleid not font:" ..role.roleid)
        return
    end
    list = SkillUpControl.GetSkillUpDataList(roleData.rank, roleData.New_Career)
    local roleLv = 0
    ...
    if roleLv <= 0 then
        ...
        curSpend = 0
        for i = roleLv - 1, 0, -1 do
            curSpend = curSpend + tonumber(list[i].cost[3])
        end
        --curSpend = tonumber(list[roleLv - 1].cost[3])--.skillcost 技能等级是加装备以后的
    end
        ...
        if tab.goods then
            for k,v in pairs(tab.goods) do
                ...
                if v.goodsID == 100001 and v.goodsType == 4 then
                    PlayerControl.GetPlayerData():PushVigor(tab.vigor)  --推送体力
                else
                    ---更新物品奖励
            ...
            ---推送收取数据
            ArkControl.PushBuildDataByReap(tab.homeInfo.homeId,tab.homeInfo.uTime)
        end
        ---更新数据统计
        TaskControl.ChangeStatistics(tab.day,tab.week,tab.month,tab.glory)
        ---远征数据更新
        local tCurBuild = ArkControl.GetCurLevelBuildData(tab.homeInfo.homeId)
        if tCurBuild.buildType == ArkControl.ArkType.expedition then
            ArkControl.InitPlayerExpeditionData(tCurBuild.taskGroup)
        end
        ---推送新建筑数据
...
---家园远征结算
function ArkViewModel.EndHomeExpeditionREQ(finishTask,hId,funACK,funNTF)
    local BaseREQ = {
        Ids = finishTask,
        homeId = hId,
    }
...
--技能书批量合成
function ArkViewModel.SendSkillMaterialsMake_Multi(id_count_Tab,callBack)
    if id_count_Tab == nil or #id_count_Tab == 0 then
        return
    end
    local t = {}
    for i,v in pairs(id_count_Tab) do
        local item = { roleID = v.id,count = v.count}
        table.insert(t,item)
    end
    local tab = {items = t}
    local bytes =  assert(pb.encode('PBClient.ClientSkillMaterialsMakeREQ',tab))
    ItemControl.AckError = true
    TaskControl.AckError = true
    ---发送数据
    MgrNet.SendReq(MID.CLIENT_SKILLMATERIALS_MAKE_REQ,bytes,0,nil,function(...)
...
function ArkViewModel.CheckTaskState()
    ...
    if #ArkViewModel.sTaskData < ArkViewModel.HomeData[ArkControl.ArkType.expedition].expeditionNum then
        return true
    end
    return false
end
...
function ArkViewModel.RefreshTaskDataStatus()
    if next(ArkViewModel.sTaskData) == nil then
        return
    end
...
function ArkViewModel.GetExpeditionNum()
    ...
    if next(ArkViewModel.sTaskData) then
        for k,v in pairs(ArkViewModel.sTaskData) do
            if v.status == 1 and ArkControl.GetExpeditionDataByID(v.id).useTime + v.uTime > Global.GetCurTime() then  --如果状态是1且在远征中
                num = num + 1
            end
        end
    end
...
function ArkViewModel.GetFinishExpeditionNum()
    ...
    if next(ArkViewModel.sTaskData) then
        for k,v in pairs(ArkViewModel.sTaskData) do
            if v.status == 1 and Global.GetCurTime() > ArkControl.GetExpeditionDataByID(v.id).useTime + v.uTime then  --如果状态是1且在远征中
                num = num + 1
            end
        end
    end
