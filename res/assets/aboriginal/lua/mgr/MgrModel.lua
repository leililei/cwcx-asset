...
function MgrModel.Install()
    ...
    require("Model/YueTa/YueTaControl")               ---月塔活动控制器
    require("Model/Time/TimeControl")                   ---时间控制器
    require("Model/Event4/Event4Control")               ---七月活动控制器
    require("Model/Activity/ActivitySkillControl")       ---活动拓展技能控制器
    require("Model/Bath/BathControl")                   ---搓澡控制器
    require("Model/LimitSign/LimitSignControl")         ---限时签到
    require("Model/HalfAnniversary/HalfAnniControl")         ---半周年
    require("Model/LimitSign/LimitSign_NewControl")         ---新限时签到活动
    require("Model/Activity/CakeControl")               ---生日签到活动
    require("Model/Budoukai/BuDoukaiControl")               ---武道会控制器
    require("Model/Skin/SkinControl")               ---皮肤控制器
    require("Core/ServerErrorCode")                     ---服务器错误码
    require("Core/ServerErrorType")                     ---服务器错误提示类型
    require("Model/PersonStory/PersonStoryControl")             ---个人剧情控制器
    require("UI/ViewModel/BlueprintViewModel")             ---个人剧情控制器
    require("Model/Invite/InviteControl")
    ---
    ---主界面根节点
...
--[[    ---签到根节点
    ...
    -- 剧情回放节点
    RedDotControl.RegisterChildDot("PlotReview","Home");
    ---指挥室根节点
    RedDotControl.RegisterChildDot("Ark","Home")
    RedDotControl.RegisterChildDot("Res","Ark")
    RedDotControl.RegisterChildDot("Expedition","Ark")
    RedDotControl.RegisterChildDot("BuildUp","Ark")
    RedDotControl.RegisterChildDot("PersonalPlot","Ark")
    ---公告根节点
    RedDotControl.RegisterChildDot("Notice","Home")
    ---活动入口根节点
    RedDotControl.RegisterChildDot("Activity","Home")
    ---商城根节点
    RedDotControl.RegisterChildDot("Shop","Home")
    ---七日签到
    ...
    ---回归签到
    RedDotControl.RegisterChildDot("SignBack","Activity")
    ---基金
    ...
    ---章节任务
    RedDotControl.RegisterChildDot("ChapterTask","Activity")
    ---战术指导
    ...
    ---预热活动
    RedDotControl.RegisterChildDot("PreheatXiGua","Activity")
    ---夏活任务
    ...
    ---月塔
    RedDotControl.RegisterChildDot("Yueta","Activity")
    RedDotControl.RegisterChildDot("YuetaReward","Yueta")
    ---累计充值
    RedDotControl.RegisterChildDot("TotalRecharge","Activity")
    ---限时充值
    RedDotControl.RegisterChildDot("MonthlyRecharge","Activity")
    ---七月活动预热
    RedDotControl.RegisterChildDot("Event4Preheat","Activity")
    RedDotControl.RegisterChildDot("Event4PreSign","Event4Preheat")
    RedDotControl.RegisterChildDot("Event4PreMap","Event4Preheat")
    ---七月活动
    RedDotControl.RegisterChildDot("Event4","Activity")
    RedDotControl.RegisterChildDot("Event4DailyTask","Event4")
    RedDotControl.RegisterChildDot("Event4Achievement","Event4")
    RedDotControl.RegisterChildDot("Event4Award","Event4")
    RedDotControl.RegisterChildDot("Event4Xingbian","Event4")
    --半周年
    RedDotControl.RegisterChildDot("HalfAnniversary","Activity")
    RedDotControl.RegisterChildDot("HalfAnniversaryPlot","HalfAnniversary")
    RedDotControl.RegisterChildDot("HalfAnniversaryTask","HalfAnniversary")
    RedDotControl.RegisterChildDot("HalfAnniversaryDailyTask","HalfAnniversaryTask")
    RedDotControl.RegisterChildDot("HalfAnniversaryAchievement","HalfAnniversaryTask")
    --抽卡
    RedDotControl.RegisterChildDot("DrawCard","Home")
    RedDotControl.RegisterChildDot("DrawCardRole","DrawCard")
    RedDotControl.RegisterChildDot("DrawCardEquip","DrawCard")
    RedDotControl.RegisterChildDot("DrawCardSpecial","DrawCard")
    ---生日活动
    RedDotControl.RegisterChildDot("Cake","Activity")
    RedDotControl.RegisterChildDot("CakeSign","Cake")
    RedDotControl.RegisterChildDot("CakeTask","Cake")
    ---武道会
    RedDotControl.RegisterChildDot("BuDouKai","Activity")
    RedDotControl.RegisterChildDot("BuDoukaiDailyTask","BuDouKai")
    RedDotControl.RegisterChildDot("BuDoukaiAch","BuDouKai")
    ---百日签到
    RedDotControl.RegisterChildDot("LimitSignAll","Activity")
    --体力消耗活动
    RedDotControl.RegisterChildDot("PowerCost","Activity")
    --累销活动(宇宙秘藏)
    RedDotControl.RegisterChildDot("CumulativeSales", "Activity")
    --累销活动/主线任务活动(领航护卫队！集结！)
    RedDotControl.RegisterChildDot("CumulativeSales2", "Activity")
    --拉新活动
    RedDotControl.RegisterChildDot("Invite","Activity")
    --Bingo
    RedDotControl.RegisterChildDot("Bingo", "Activity")
end

---登录推送数据
function MgrModel.PushData(data, isReconnet)
    TimeControl.Init(data.timeInfo)
    ...
    EquipControl.CreateAllSpecialEquip(data.SEquips)            ---进阶共鸣装备
    ...
    StormControl.SaveWeekBossTime(data.weekCopyNum, data.weekCopyNumTotal)             ---更新周本次数
    ...
    CardDrawControl.Init(data)                                      ---初始化卡池数据
    ...
    require("UI/ViewModel/HomeViewModel").CollationPackages(data.PayPackInfoS)
    StormViewModel.SendStormBossData2()                         ---初始化联合讨伐
    FundControl.Init()                                          ---基金
    SkillUpControl.InitSkillUpData()                            ---技能升级消耗
    HaiYueControl.Init()                                        ---海月活动
    Event4Control.Init()                                        ---七月活动
    ActivitySkillControl.Init()                                 ---七月活动
    BathControl.Init()
    LimitSignControl.Init()
    HalfAnniControl.Init()                                      ---半周年
    LimitSign_NewControl.Init()
    CakeControl.Init()                                          ---生日签到活动
    BuDoukaiControl.Init()                                      ---武道会
    SkinControl.InitData()                                      ---皮肤
    PersonStoryControl.Init()                                   --个人剧情
    ItemControl.PushLimitGoods(data.limitGoods)                 --初始化限时道具
    BlueprintViewModel.SetWeekRaidNum(data.bpWeekRaidNum);
    InviteControl.Init()                                        --拉新活动
end
...
function MgrModel.ClearAll()
    ...
    HaiYueControl.Clear()
    YueTaViewModel.Clear()
    Event4Control.Clear()
    ActivitySkillControl.Clear()
    BathControl.Clear()
    LimitSignControl.Clear()
    HalfAnniControl.Clear()
    YueTaControl.Clear()
    CakeControl.Clear()
    BuDoukaiControl.Clear()
    SkinControl.Clear()
    BattleViewModel.Clear()
    PersonStoryControl.Clear()
    InviteControl.Clear()
    CommonViewModel.Clear()
    BingoViewModel.Clear()
    CumulativeSalesViewModel.Clear()
end
