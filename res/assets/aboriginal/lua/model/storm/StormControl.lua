...
require("LocalData/AbysslevelLocalData")
...
---@type StormScrollData[] 挑战月塔卷信息
local MonthTowerScrollDataList = {}
---@type StormPointData[] 挑战月塔关卡信息
local MonthTowerPointDataList = {}
...
local StormWeekBossTime = 0
local StormWeekBossTotalTime = 0

---@type StormBossData[] 联合讨伐（世界boss）信息
local StormBossList = {}
---连战场次
local BattleContinuNum = 1
...
function StormControl.CreateAllStorm()
    ...
    ---初始化月塔关卡
    pointIDStr = ""
    for id, config in pairs(AbysslevelLocalData.tab) do
        MonthTowerPointDataList[id] = StormPointData.New(id)
        MonthTowerPointDataList[id]:PushMonthTowerConfig(config)
        pointIDStr = pointIDStr..id..","
    end
    ---初始化月塔卷
    MonthTowerScrollDataList[1] = StormScrollData.New(1)
    MonthTowerScrollDataList[1]:PushMonthTower(string.sub(pointIDStr,1,#pointIDStr - 1))
    ---初始化挑战战术指导关卡
    pointIDStr = ""
...
---@return StormScrollData[] 获取月塔卷
function StormControl.GetMonthTowerScrollData()
    return MonthTowerScrollDataList
end
---@return StormPointData[] 获取月塔关卡
function StormControl.GetMonthTowerPointData()
    local arr = {}
    for i,v in pairs(MonthTowerPointDataList) do
        if YueTaControl.GetYueTaInfo() ~= nil and v.group == YueTaControl.GetYueTaInfo().chapterGroup then
            arr[v.id] = v
        end
    end
    return arr
...
function StormControl.GetHighestPoint()
    ...
    for i,v in pairs(StormScrollDataList) do
        if(StormControl.CheckScrollLock(v.id) and v.type == 0) then
            ---防止重复遍历 小的盖了大的
            if v.raidType ~= 999 and (v.id > index) then
...
function StormControl.SaveWeekBossTime(num, totalNum)
    StormWeekBossTime = CheckNumber(num)
    StormWeekBossTotalTime = CheckNumber(totalNum)
end

function StormControl.GetWeekBossTime()
    return StormWeekBossTime
end

function StormControl.GetWeekBossTotalTime()
    return StormWeekBossTotalTime
end

---设置连战场次
function StormControl.SetBattleContinu(_num)
    BattleContinuNum = _num
end
function StormControl.GetBattleContinu()
    return BattleContinuNum
end

--region 关卡奖励数据

---@class NDATA_DropItem
---@field dropID int
---@field num int @ 奖励数量，应该指的是抽取次数
---@field dropGoods string @ 奖励奖池，是整个池子，不是最终发放

---@class NDATA_LevelDropInfo
---@field levelID int
---@field reward string @ 三星首通奖励
---@field drop NDATA_DropItem[] @ 随机奖励，池子数据，非最终发放
---@field victory goods[] @ 通关奖励
---@field roleEXP goods[] @ 角色经验
---@field failure goods[] @ 失败奖励
---@field levelCost goods[] @ 关卡消耗

---@param infos NDATA_LevelDropInfo[]
function StormControl.PushLevelDropInfo(infos)
    for _, infoData in pairs(infos) do
        local pointData = StormControl.GetStormPointByID(infoData.levelID)
        if pointData then
            if infoData.reward then
                pointData:PushRewardInfo(nil, infoData.reward)
            end
            if infoData.victory then
                pointData:PushVictoryInfo(infoData.victory, nil)
            end
            if infoData.roleEXP then
                pointData:PushRoleExpInfo(infoData.roleEXP, nil)
            end
            if infoData.failure then
                pointData:PushFailureInfo(infoData.failure, nil)
            end
            if infoData.levelCost then
                pointData:PushConsumeInfo(infoData.levelCost, nil)
            end
            
            if infoData.drop then
                --- 要计算倍率
                ---@type goods[]
                local dropList = {}
                for _, drop in pairs(infoData.drop) do
                    local powerNum = drop.num
                    local dropGoods = Global.StrToGoods(drop.dropGoods)
                    for _, dg in ipairs(dropGoods) do
                        dg.goodsNum = dg.goodsNum * powerNum
                        
                        local bNewGoods = true
                        for _, hg in ipairs(dropList) do
                            -- 已经有该物品，增加数量
                            if hg.goodsType == dg.goodsType and hg.goodsID == dg.goodsID then
                                bNewGoods = false
                                hg.goodsNum = hg.goodsNum + dg.goodsNum
                                break
                            end
                        end
                        -- 否则插入新物品
                        if bNewGoods then
                            table.insert(dropList, dg)
                        end
                    end
                end
                pointData:PushRandomDropInfo(dropList)
            end
        else
            Global.LogError(string.format("未能找到关卡(%s)数据，无法写入关卡奖励信息", infoData.levelID))
        end
    end
end
