...
local M = Class('TowerPointItem', UIItemBase)
function M:ctor()
    M.super.ctor(self)
    self.PathPrefab = 'ABOriginal/UI/Prefab/Template/TowerPointItem.prefab'
    -- 没有使用组建缓存列表
    self.CC = {
        -- Image 列表
        ...
        -- UITemplate 列表
        {'TowerPointItem','/',10},{'TowerRoleItem_1','Panel/Panel_Info/RoleContent/TowerRoleItem_1',10},{'TowerRoleItem_2','Panel/Panel_Info/RoleContent/TowerRoleItem_2',10},{'TowerRoleItem_3','Panel/Panel_Info/RoleContent/TowerRoleItem_3',10},{'TowerRoleItem_4','Panel/Panel_Info/RoleContent/TowerRoleItem_4',10},{'TowerRoleItem_5','Panel/Panel_Info/RoleContent/TowerRoleItem_5',10},{'TowerRoleItem_6','Panel/Panel_Info/RoleContent/TowerRoleItem_6',10},{'TowerRoleItem_7','Panel/Panel_Info/RoleContent/TowerRoleItem_7',10},{'TowerRoleItem_8','Panel/Panel_Info/RoleContent/TowerRoleItem_8',10},
        -- RawImage 列表
        {'TowerPointItem01','/',15},{'IsAdopt','Panel/IsAdopt',15},
        -- TextMeshProUGUI 列表
        {'Text_Yitongguan','Panel/IsAdopt/Text_Yitongguan',20},{'Text_Clear','Panel/IsAdopt/Text_Clear',20},{'Text_Number','Panel/Text_Number',20},
    }
end
...
function M:OnUpdateUI(pData)
    ...
    if not self.pData.isEmpty then
        ---更新当前层数
        self.Text_Number().text = string.format(MgrLanguageData.GetLanguageByKey("towerpointitem_floor"),self.pData.id % 1000 + 1)
        ---是否解锁
        ...
        ---是否完成所有条件
        local clear = true
        for i, v in ipairs(self.pData:CheckTowerTask()) do
            if v ~= true then
                clear = false
                break
            end
