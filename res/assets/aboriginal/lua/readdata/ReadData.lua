...
---核心和共鸣装备都是自己的数据
function ReadData.CreatRole(GameDataID,skinID,lv,startLV,skillLV,Bool_isAwaken,favor,isPVPRoleOrFriendRole,equipUnlockLV,heroFlag,heroCurSkill)
    ReadData.CeratExp()
    print("----------------------------------------------------")
    print(GameDataID)
    print(Bool_isAwaken)
    ...
    tempRole.New_Career = tonumber(RoleRData[3])
    ...
    --技能等级设置
    if heroFlag ~= nil then
        tempRole.heroFlag = heroFlag
        if heroCurSkill == nil then
            heroCurSkill = skillLV
        else
            tempRole.heroCurSkill = heroCurSkill
        end
    end
    if heroFlag ~= nil and heroFlag ~= 0 and heroFlag ~= false then
        tempRole.ShowSkillLV = heroCurSkill
    end
    --立绘偏移xy
    ...
    tempRole.TabBuffSkill={}
    tempRole.BattleBuffList={}
    BlueprintViewModel.AddBlueprintBaseProps(tempRole);
    return tempRole
end

function ReadData.ReplaceSkill(skillStr,tempRole,index)
    local roleData = HeroControl.GetRoleDataByID(tonumber(tempRole.ID))
    if roleData and not roleData.sEquipOff then
        return
    end
    local realStr=BattleRole.RealSkillStr(skillStr, tempRole.ShowSkillLV,tempRole,index)
    tempRole["Skill_"..index.."_IsSLv"] = false
    tempRole["Skill_"..index.."Fml"]=JNStrTool.StrArrArr("@",realStr,{"{" , "}" , "," })
    tempRole["Skill_"..index]=BattleRole.SubSkillLvReal2(tempRole["Skill_"..index.."Fml"],tempRole.ShowSkillLV)
...
function ReadData.CreatMonster(GameDataID,lv,startLV,skillLV,Bool_isAwaken,_qoom,int_atkOrder)
    ...
    tempRole.monsterAtkOrder = int_atkOrder
    tempRole.AllNumber_Out=0
    tempRole.AllNumber_In=0
    tempRole.AllHPNumber_Out=0
    tempRole.AllHPNumber_In=0
...
function ReadData.CreatMonster_SummonObj(GameDataID,lv,startLV,skillLV,Bool_isAwaken,_qoom,coreID1,core1Po,core1Skill,coreID2,core2Po,core2Skill,int_atkOrder)
    if Bool_isAwaken==nil or  Bool_isAwaken=="1"or  Bool_isAwaken==1 or  Bool_isAwaken==true  then
        Bool_isAwaken=true
    else
        Bool_isAwaken=false
    ...
    ---添加召唤物所有装备属性
    ReadData.SetMabt(tempRole)
    local AbtArr1 = ReadData.GetGearAttr(coreID1,core1Po)
    if AbtArr1 then
        ReadData.InitRoleGear( tempRole, AbtArr1 ,true)
    end
    local AbtArr2 = ReadData.GetGearAttr(coreID2,core2Po)
    if AbtArr2 then
        ReadData.InitRoleGear( tempRole, AbtArr2 ,true)
    end
    ---记录召唤物装备ID
    tempRole.coreID1 = coreID1
    tempRole.core1Po = core1Po
    tempRole.coreID2 = coreID2
    tempRole.core2Po = core2Po
...
function ReadData.GetRoleAttr(RoleId,lv,starLV,skillLV,Bool_isAwaken,exp,equipUnlockLV)
    ...
    ---重点技能组(技能树显示用)
    tempRole.PointSkillGroup = RoleRData[61]
    tempRole.TabBuffSkill={}
    tempRole.BattleBuffList={}
    ---觉醒后增加属性
    tempRole.StSAwaken(RoleRData[28],tempRole)
...
function ReadData.GetMonsterAttr(RoleId,startLV,skillLV,Bool_isAwaken,Lv)
    ...
    tempRole.AtkTargetTips = ReadData.GetAtkTargetTips(RoleRData[30])
    tempRole.Attacktarget = RoleRData[30]
    ---技能
...
function ReadData.SetAdd_Abt(_role ,_str_type,_number)
    if nil == _role.extraAtk_Fixed then
        _role.extraAtk_Fixed = 0;
    end
    if _str_type==0 then  --固定攻击
        _role.basisAtk_Fixed=_role.basisAtk_Fixed+_number
        --实际攻击为  (basisAtk+basisAtk_Fixed)*basisAtk_Percent
        --然后给基础属性赋值
        _role.Atk= (_role.basisAtk+_role.basisAtk_Fixed)*(1+ _role.basisAtk_Percent) + _role.extraAtk_Fixed;
    ...
    elseif _str_type == 9 then  --额外攻击力
        _role.extraAtk_Fixed = _role.extraAtk_Fixed + _number;
        _role.Atk = (_role.basisAtk + _role.basisAtk_Fixed) * (1 + _role.basisAtk_Percent) + _role.extraAtk_Fixed;
        _role.RealAtk = _role.Atk
    end
end
...
function ReadData.SetAllFw_attr(_Role)
    ...
    if _Role.IsMonster then
        ...
        if BattleManager.FightType ~= nil then
            if CJNBattleMgr.Instance == nil or not CJNBattleMgr.Instance.worldBossBattle then
                ---常规战斗
                data = StormViewModel.CurPointData:GetMonsterById(tonumber(_Role.ID),_Role.monsterAtkOrder)
            elseif CJNBattleMgr.Instance.worldBossBattle then
                ---世界boss
                data = EventRaidControl.GetLIANHETAOFAData().BossData[BattleManager.CurBossLevel].monsterData[1][_Role.monsterAtkOrder]
            end
        end
    else
        ---人物
    ...
    ---获取角色核心1
    local core1 = data:GetHeroCore(1,ArmorType)
    if core1 ~= nil then
        ---添加核心技能
    ...
    local core2 = data:GetHeroCore(2,ArmorType)
    if core2 ~= nil then
        ---添加核心技能
    ...
    if not _Role.IsMonster then
        ---如果解锁进阶装备,则替换原有共鸣装备属性
        if data:GetHeroSpecialEquip() == nil or not data:GetHeroSpecialEquip():CheckLockState() or not data.sEquipOff then
            ---获取角色共鸣装备1
            ...
            local specialEquip = data:GetHeroSpecialEquip()
            if specialEquip ~= nil then
                ReadData.InitRoleGear(_Role,specialEquip:GetCurrentAttr(),true)
            end
        end
    end
end
...
function ReadData.GetAtkTargetTips(_idx)
    ...
    elseif _idx == 6 then
        return MgrLanguageData.GetLanguageByKey("fightdragview_min")
    end
end
--添加BOSS所有的装备属性
...
---设置核心类型(0进攻 1防守)
function ReadData.SetCoreType(_armorType)
