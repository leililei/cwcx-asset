...
function ShopItemData:ctor()
    ...
    self.bgIcon = ""
    self.buyLimit = 0
    self.DiscountSTime = 0
    self.DiscountETime = 0
end
...
function ShopItemData:PushConfig(config)
    ...
    self.sellType = config.type     --0:永久出售 1:限时出售 2:弹出礼包 3:限时打折
    local timeData = TimeControl.GetTimeTable(config.time_open)
    if timeData == nil then
        UnityEngine.Debug.LogError("服务器没有该时间  " .. config.time_open)
    end
    self.sellSTime = timeData.openTime
    self.sellETime = timeData.endTime
    self.buyCount = 0
...
function ShopItemData:GetResetBuyTime(time)
    ...
    local reset = false
    --当前时间
    local curTime = os.date("!*t",time + tonumber(SteamLocalData.tab[112007][2]) * 3600)     --进入商店时的服务器时间
    ---1、每日重置 2、每周重置 3、每月重置 4、每年重置 其他不重置
    if self.resetBuyType == 1 then
        --日刷新时间配置
        local conf = string.split(SteamLocalData.tab[112001][2],":")
        --如果当天刷新，当天刷新时间戳
        local rTime = os.time({year = curTime.year, month = curTime.month, day = curTime.day, hour = curTime.hour >= tonumber(conf[1]) and 29 or tonumber(conf[1]) , min = tonumber(conf[2]), sec = tonumber(conf[3])}) - (tonumber(SteamLocalData.tab[112007][2]) - timeZone) * 3600
        --下次刷新时间
    ...
    elseif self.resetBuyType == 2 then
        ...
        --如果当天刷新，当天刷新时间戳
        local rTime = os.time({year = curTime.year, month = curTime.month, day = curTime.day, hour = tonumber(timeStr[1]) , min = tonumber(timeStr[2]), sec = tonumber(timeStr[3])}) - (tonumber(SteamLocalData.tab[112007][2]) - timeZone) * 3600
        --本周第几天
        local day = tonumber(os.date("%w",time + (tonumber(SteamLocalData.tab[112007][2]) - Global.GetClientTimeZone()) * 3600)) == 0 and 7 or tonumber(os.date("%w",time + (tonumber(SteamLocalData.tab[112007][2]) - Global.GetClientTimeZone()) * 3600))
        --本周刷新日
        local targetTime = os.date("!*t",rTime - ((day - weekNum) * 86400) + 604800 + tonumber(SteamLocalData.tab[112007][2]) * 3600)
        --下次刷新时间
    ...
    elseif self.resetBuyType == 3 then
        ...
        local rTime = os.time({year = curTime.year, month = curTime.month, day = curTime.day, hour = tonumber(timeStr[1]) , min = tonumber(timeStr[2]), sec = tonumber(timeStr[3])}) - (tonumber(SteamLocalData.tab[112007][2]) - timeZone) * 3600
        --当前月份数
        local monthNum = tonumber(os.date("%m",time + (tonumber(SteamLocalData.tab[112007][2]) - Global.GetClientTimeZone()) * 3600))
        --当前年份数
        local yearNum = tonumber(os.date("%Y",time + (tonumber(SteamLocalData.tab[112007][2]) - Global.GetClientTimeZone()) * 3600))
        --本月第几天
        local dayNum = tonumber(os.date("%d",time + (tonumber(SteamLocalData.tab[112007][2]) - Global.GetClientTimeZone()) * 3600))
        --下次刷新时间
        local nextTime = 0
        if monthNum == 12 then
            --如果当前12月，次月刷新时间就是下一年的1月份
            nextTime = os.time({year = yearNum + 1, month = 1, day = dayAmount, hour = tonumber(timeStr[1]) , min = tonumber(timeStr[2]), sec = tonumber(timeStr[3])}) + (tonumber(SteamLocalData.tab[112007][2]) - Global.GetClientTimeZone()) * 3600
        else
            --正常月份+1
            nextTime = os.time({year = yearNum, month = monthNum + 1, day = dayAmount, hour = tonumber(timeStr[1]) , min = tonumber(timeStr[2]), sec = tonumber(timeStr[3])}) + (tonumber(SteamLocalData.tab[112007][2]) - Global.GetClientTimeZone()) * 3600
        end
        --当前要刷新
    ...
    elseif self.resetBuyType == 4 then
        ...
        local dayNum = tonumber(os.date("%d",time + (tonumber(SteamLocalData.tab[112007][2]) - Global.GetClientTimeZone()) * 3600))
        --下一年刷新时间
        local nextTime = os.time({year = curTime.year + 1, month = monthAmount, day = 1, hour = tonumber(timeStr[1]) , min = tonumber(timeStr[2]), sec = tonumber(timeStr[3])}) + (tonumber(SteamLocalData.tab[112007][2]) - Global.GetClientTimeZone()) * 3600
        --当天是刷新日
...
---获取商品下架时间
function ShopItemData:GetGoodsCutTime()
    if self.sellType == 0 or not Global.isMiddleTime(self.sellSTime, self.sellETime) then
        return nil
    end
    local tCurTime = Global.GetCurTime()
    local tEndTime = self.sellETime
    local CutTime = tEndTime - tCurTime
...
---商品限时打折时间
function ShopItemData:ItemDiscountTime(discountTime)
    self.sellSTime = discountTime.saleStartTime
    self.sellETime = discountTime.saleEndTime
    self.DiscountSTime = discountTime.discountStartTime
    self.DiscountETime = discountTime.discountEndTime
end
function ShopItemData:IsDisCounting()
    if self.DiscountSTime == 0 and self.DiscountETime == 0 then
        return true
    end
    if Global.isMiddleTime(self.DiscountSTime,self.DiscountETime) then
        return true
    else
        return false
    end
end
function ShopItemData:SoldOut()
    if 0 == self.buyMaxCount then
        return false;
    end
    return self.buyCount >= self.buyMaxCount;
end
--- 商品是否应该显示红点
---@return boolean
function ShopItemData:IsShowRedDot()
    ...
    if price.goodsNum == 0 then
        return true
    end
    
    -- 目前仅免费礼包显示红点
    --local isNew = UnityEngine.PlayerPrefs.GetString(string.format("%sStoreTimeGift%s", PlayerControl.GetPlayerData().UID, self.id))
