...
function IllustrationViewModel.OpenRoleXiangqing(id,ids,noSkin)
    IllustrationViewModel.AllHeroCacheData = HeroControl.GetAllHero()
    local role
    local roles = {}
    ...
    --不显示皮肤
    if noSkin then
        if role2.skin and role2.skin ~= role2.id then
            role2.skin = role2.id
        end
        for k,v in pairs(roles2) do
            if v.skin and v.skin ~= v.id then
                v.skin = v.id
            end
        end
    end
...
function IllustrationViewModel.GetMetalRatio()
    local all = #ItemControl.GetHideDataListMedal()
