...
---@type StormScrollData[] 月塔卷信息缓存
StormViewModel.CacheMonthTowerScrollData = {}
---@type StormPointData[]  月塔关卡缓存
StormViewModel.CacheMonthTowerPointData = {}
...
---关卡类型
---@alias StormViewModel.PointType int
...
StormViewModel.PointBattleType = {
    ...
    ACTIVITY_GUIDANCE = 107,        ---活动指导关卡
    ACTIVITY_BOSSGUIDANCE = 108,    ---活动BOSS指导关卡
    ACTIVITY_SEAT = 200,            ---活动类型末尾占位
...
function StormViewModel.OpenStormPointUI(data,type)
    if data.raidType == 13 and not SysLockControl.CheckSysLock(1111) then
...
--- 简易包装，若有其它需求请自行调用 RequestLevelDropInfo
---@param data StormScrollData
---@param type StormViewModel.PointType
function StormViewModel.OpenStormPointUIAfterRequestDrop(data, type)
    StormViewModel.RequestLevelDropInfo(data.points, function() 
        StormViewModel.OpenStormPointUI(data, type)
    end)
end
    ...
    then
        ...
        then    --活动Boss结算
            if BattleViewModel.NormalRewardTab ~= nil and StormViewModel.CurPointData.battleMap ~= "0" then
                ...
                MgrUI.Pop(UID.PVEComplete_UI,{BattleViewModel.NormalRewardTab,offsetExp,offsetExpList,true},true)
            else
                MgrUI.Pop(UID.PVEComplete_UI,{BattleViewModel.NormalRewardTab,nil,nil,true},true)
            end
        else
            ---纯剧情关卡直接通关
            if StormViewModel.CurPointData.battleMap == "0" then
                if StormViewModel.CurPointData.type == StormViewModel.PointBattleType.STORY then
                    if StormViewModel.CurPointData.star > 0 then
                        StormViewModel.CheckNextPoint()
                    else
                        StormViewModel.GetPlotReward()
                    end
                else
                    if (StormViewModel.CurPointData.type > StormViewModel.PointBattleType.LOGIC_BATTLE and StormViewModel.CurPointData.star > 0)then
                        ---已经通关
                        MgrBattle.CloseFight()
                        ---检查当前是否是最后一关并且为剧情
                ...
                if BattleViewModel.NormalRewardTab ~= nil then
                    ---显示结算
                    local offsetExp = BattleViewModel.NormalRewardTab.userExp - BattleViewModel.playerExp
                    local offsetExpList = {}
                    if BattleViewModel.NormalRewardTab.heroInfos ~= nil then
                    ...
                    if StormViewModel.CurPointType == StormViewModel.PointType.activityBoss then
                        ...
                        if BattleManager.GameMode == BattleManager.GameModeType.ActivityBoss then
                            ...
                            if StormViewModel.CurPointData.type == StormViewModel.PointBattleType.LOGIC_BATTLE or StormViewModel.CurPointData.type == StormViewModel.PointBattleType.LOGIC_BOSS then
                                MgrUI.Pop(UID.PVEComplete_UI,{BattleViewModel.NormalRewardTab,offsetExp,offsetExpList,vigorRefund},true)
                                return
                            end
                            MgrUI.Pop(UID.PVEMainComplete_UI,{BattleViewModel.NormalRewardTab,offsetExp,offsetExpList,vigorRefund},true)
                        end
                    end
                end
            end
...
function StormViewModel.ReloadStormData()
    ...
    ---获取月塔关卡缓存
    StormViewModel.CacheMonthTowerPointData = StormControl.GetMonthTowerPointData()
    ---获取月塔卷缓存
    StormViewModel.CacheMonthTowerScrollData = StormControl.GetMonthTowerScrollData()
    ---获取红巨关卡缓存
...
function StormViewModel.ReqBossDataNtf2(buffer,tag)
    ...
    if data ~= nil then
        ...
        StormViewModel.CurStormBossId = tab.bossID
        StormViewModel.CurStormAnaBossId = tab.bossID
    else
    ...
    if tab.goods ~= nil and tab.goods ~= {} then
        ---检查是否未领取
        BattleViewModel.LHTFReward = tab.goods
        ItemControl.PushGroupItemData(tab.goods,ItemControl.PushEnum.add)
    end
    ---更新所有boss历史分数
    if tab.maxScores ~= nil then
    ...
    StormViewModel.SelfRankInfo = {
        score = tab.score,
        rank = tab.nowRank,
        nike = PlayerControl.GetPlayerData().nickname,
        head = PlayerControl.GetPlayerData().headUrl,
        headFrame = PlayerControl.GetPlayerData().headFrame,
        level = PlayerControl.GetPlayerData().level,
        count = tab.count,
        title = PlayerControl.GetPlayerData().title,
        id = PlayerControl.GetPlayerData().UID,
        todayScore = tab.todayScore,
        array = tab.array,
...
function StormViewModel.LHTFRewardFunc()
    ...
    if BattleViewModel.LHTFReward ~= nil and #BattleViewModel.LHTFReward ~= 0 then
        MgrUI.Pop(UID.ItemAchievePop_UI,{BattleViewModel.LHTFReward},true)
    end
end
...
function StormViewModel.GetSortAndFilterHeroArr(filters,sort,rise,isSupport)
    ...
    local personArray = PersonStoryControl.GetSortAndFilterHeroArr(array,filters,sort,rise)
    if next(personArray) then
        return personArray
    end
    ---有类型时筛选
...
function StormViewModel.AddHeroData(heroId)
    if StormViewModel.CurPointType == StormViewModel.PointType.guide or(StormViewModel.CurPointData ~= nil and StormViewModel.JudgeTeachType(StormViewModel.CurPointData.type))  then
...
function StormViewModel.GetStormPointData(skipBlank)
...
---@return StormPointData[] 获取挑战月塔层数据
function StormViewModel.GetMonthTowerPointData(groupID,hard)
    local arr = {}
    for i, v in pairs(StormViewModel.CacheMonthTowerPointData) do
        if v.group == groupID and v.hard == hard then
            arr[#arr + 1] = v
        end
    end
    ---下方添加3条空数据做居中效果
...
---@return number 获取月塔数量
function StormViewModel.GetMonthTowerCount(groupID,hard)
    local count = 0
    for i, v in pairs(StormViewModel.CacheMonthTowerPointData) do
        if v.group == groupID and v.hard == hard then
            count = count + 1
...
function StormViewModel.PushYuetaPointData(towerStage)
    if towerStage then
        for i,v in pairs(towerStage) do
            for j,towerPoint in pairs(StormViewModel.CacheMonthTowerPointData) do
                if towerPoint.id == v.level then
                    towerPoint.isPass = v.status ~= 0
                end
            end
        end
    end
...
function StormViewModel.PushYuetaInnerPointData(towerStage)
    if towerStage then
        for i,v in pairs(towerStage) do
            for j,towerPoint in pairs(StormViewModel.CacheMonthTowerPointData) do
                if towerPoint.id == v.level then
...
function StormViewModel.ResetYuetaInnerPointData()
    for j,towerPoint in pairs(StormViewModel.CacheMonthTowerPointData) do
        if YueTaControl.GetYueTaInfo().chapterGroup == towerPoint.group and towerPoint.hard == 1 then
            towerPoint.isPass = false
        end
    end
end
...
function StormViewModel.JudgeTeachType(type)
    if type == 999 or type == 107 or type == 108 then
        return true
    else
        return false
    end
...
function StormViewModel.GetPlotReward()
    ...
    local thisACK = function(buffer,tag)
        ...
        if tab.errNo ~= 0 then
            MgrUI.Pop(UID.ClosePop_UI,{MgrLanguageData.GetErrorByKey(tab.errNo), function ()
                MgrSdk.BackToLogin()
            end},true)
        end
    end
...
function StormViewModel.CheckNextPoint()
    local data = StormViewModel.CurPointData
    if (data.type ~= StormViewModel.PointBattleType.STORY and data.type ~= StormViewModel.PointBattleType.ACTIVITY_PLOT) or data.o_fronts == "0" then
        MgrBattle.CloseFight()
        return
    end

    --检测解锁关卡是否满足条件
    if data.o_fronts ~= "0" then
        local unlockPoint = string.split(data.o_fronts,'_')
        --获取关卡数据
        local pointData = StormControl.GetStormPointByID(tonumber(unlockPoint[1]))
        ...
        if pointData and pointData.activitypreview2 ~= "0" then
            local strItem = string.split(pointData.activitypreview2, "_")
            local itemData = ItemControl.GetItemByIdAndType(tonumber(strItem[2]),tonumber(strItem[1]))
            if itemData.count < tonumber(strItem[3]) then
                MgrBattle.CloseFight()
                return
            end
        end
    ...
    MgrUI.Pop(UID.NextPointPop_UI,{MgrLanguageData.GetLanguageByKey("stormviewmodel_tips7"),function()
...
function StormViewModel.GetNextPointInfo()
    local data = StormViewModel.CurPointData
    local frontStr = string.split(data.o_fronts,"_")
    local pointId = frontStr[1]
    StormViewModel.CurPointState = StormViewModel.PointState.wait
    StormViewModel.CurPointData = StormControl.GetStormPointByID(tonumber(pointId))
    if data.scrollID ~= CheckpointLocalData.tab[tonumber(frontStr[1])].scroll then
        StormViewModel.CurScrollData = StormControl.GetStormScrollById(tonumber(frontStr[2]))
    end
end
...
---@param levelIds int[]
---@param callback fun()?
function StormViewModel.RequestLevelDropInfo(levelIds, callback)
    if levelIds == nil or #levelIds == 0 then
        Global.LogError("关卡列表为空，不执行请求关卡奖励")
        if callback then
            callback()
        end
        return
    ...
    local req = assert(pb.encode("PBClient.ClientLevelDropInfoREQ", {
        levelID = levelIds,
    }))
    MgrNet.SendReq(MID.CLIENT_LEVEL_DROP_INFO_REQ, req, 0, nil, function(buffer, tag) 
        MgrNet.CommonACK("PBClient.ClientLevelDropInfoACK", buffer, tag)
    end, function(buffer, tag) 
        local ntf = assert(pb.decode("PBClient.ClientLevelDropInfoNTF", buffer))
        StormControl.PushLevelDropInfo(ntf.Info)
        if ntf.weekCopyNum then
            StormControl.SaveWeekBossTime(ntf.weekCopyNum, ntf.weekCopyNumTotal)
        end
        if callback then
            callback()
        end
