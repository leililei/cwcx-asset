...
---设置VM
require("LocalData/LoadingimgLocalData")
SettingViewModel = {}

local UnLockLoadingImages = {}
local AllLoadingImages = {}
local ChoosenLoadingImages = {}
...
function SettingViewModel.Init()
    ...
    SettingViewModel.SetMainRoleSound(SettingViewModel.GetMainRoleSound())
    SettingViewModel.SetPostGirlSetting(SettingViewModel.GetPostGirlSetting())
    SettingViewModel.SetSkinState(SettingViewModel.GetSkinState())
end
function SettingViewModel.LoadingImageInit()
    UnLockLoadingImages = {}
    AllLoadingImages = {}
    ChoosenLoadingImages = {}
    for i,v in pairs(LoadingimgLocalData.tab) do
        local item = ItemControl.GetItemByIdAndType(v.itemunlock, 4)
        if item then
            table.insert(AllLoadingImages,v)
        end
        if item.count ~= 0 or item.id == 0 then
            ---已解锁
            table.insert(UnLockLoadingImages,v)
            --UnLockLoadingImages[v.itemunlock] = v
        end
        if UnityEngine.PlayerPrefs.GetInt("loadingImage"..v.itemunlock..PlayerControl.GetPlayerData().UID) == 1 then
            SettingViewModel.SetChoosenLoadingImages(v)
        end
    end
    ---判断选中个数,如果为0,则选中默认图片
    local chooseNum = 0
    for i,v in pairs(ChoosenLoadingImages) do
        chooseNum = chooseNum + 1
    end
    if chooseNum == 0 then
        SettingViewModel.SetChoosenLoadingImages(LoadingimgLocalData.tab[1])
    end
end

--设置cg图
function SettingViewModel.SetLoadingImage(id)
    local t
    for j,k in pairs(LoadingimgLocalData.tab) do
        if k.itemunlock == id then
            t = k
            break
        end
    end
    if t then
        table.insert(UnLockLoadingImages,t)
    end
end


function SettingViewModel.SetChoosenLoadingImages(picture)
    ChoosenLoadingImages[picture.itemunlock] = picture
end
function SettingViewModel.RemoveChoosenLoadingImages(picture)
    if ChoosenLoadingImages[picture.itemunlock] ~= nil then
        ChoosenLoadingImages[picture.itemunlock] = nil
    end
end
function SettingViewModel.ReturnChoosenLoadingImages()
    return ChoosenLoadingImages
end
function SettingViewModel.ReturnAllLoadingImages()
    return AllLoadingImages
end
function SettingViewModel.ReturnLoadingImages()
    table.sort(UnLockLoadingImages,function(a,b)
        return a.sort < b.sort
    end)
    return UnLockLoadingImages
end
function SettingViewModel.ReturnUnlockRandomImage()
    local keys = {}
    local number = 0
    for i,v in pairs(ChoosenLoadingImages) do
        table.insert(keys,i)
    end
    number = math.random(1,#keys)
    return ChoosenLoadingImages[keys[number]]
end

function SettingViewModel.ChooseNumber()
    local number = 0
    for i,v in pairs(ChoosenLoadingImages) do
        number = number + 1
    end
...
function SettingViewModel.ChooseUnlockNumber()
    local number = 0
    for i,v in pairs(UnLockLoadingImages) do
        number = number + 1
    end
    return number
end
...
function SettingViewModel.SetBattleSpeed(value)
    ...
    elseif value == 2 then
        CJNBattleMgr.SetGameSpeed(2.2)
    elseif value == 3 then
        CJNBattleMgr.SetGameSpeed(4)
    end
end
function SettingViewModel.AddBattleSpeed()
...
---@return number 获取Ex技能动画 1开启、2一次、3关闭
...
function SettingViewModel.AddExAnim()
    local value = SettingViewModel.GetExAnim();
    value = value + 1;
    if value > 3 then
        value = 1;
    end
    SettingViewModel.SetExAnim(value);
    return value;
end
...
function SettingViewModel.GetAllSound()
    local s = UnityEngine.PlayerPrefs.GetInt("St_AllSound")
    if s == nil or s == 0 then
...
---获取返回主界面人物语音
function SettingViewModel.GetMainRoleSound()
    local s = UnityEngine.PlayerPrefs.GetInt("St_MainRoleSound")
    if s == nil or s == 0 then
        s = 1
    end
    return s
...
function SettingViewModel.SetAllSound(value)
    UnityEngine.PlayerPrefs.SetInt("St_AllSound",value)
    if value == 1 then
...
---获取返回主界面人物语音
function SettingViewModel.SetMainRoleSound(value)
    UnityEngine.PlayerPrefs.SetInt("St_MainRoleSound",value)
end
---@return number 获取背景音乐 0~100
function SettingViewModel.GetBGMSound()
    local s = UnityEngine.PlayerPrefs.GetString("St_BGMSound")
    if s == "" then
...
function SettingViewModel.SetBGMSound(value)
    UnityEngine.PlayerPrefs.SetString("St_BGMSound",tostring(value))
    MgrSound.SetBGMVol(value/100)
end
---@return number 获取音效音量 0~100
function SettingViewModel.GetEffectSound()
    local s = UnityEngine.PlayerPrefs.GetString("St_EffectSound")
    if s == "" then
...
function SettingViewModel.SetEffectSound(value)
    UnityEngine.PlayerPrefs.SetString("St_EffectSound",tostring(value))
...
function SettingViewModel.GetRoleSound()
    local s = UnityEngine.PlayerPrefs.GetString("St_RoleSound")
    if s == "" then
...
function SettingViewModel.SetRoleSound(value)
    UnityEngine.PlayerPrefs.SetString("St_RoleSound",tostring(value))
...
function SettingViewModel.SetPostGirlSetting(value)
    local uid = PlayerControl.GetPlayerData().UID
    UnityEngine.PlayerPrefs.SetInt("St_PG"..uid,value)
end

function SettingViewModel.GetPostGirlSetting()
    local uid = PlayerControl.GetPlayerData().UID
    local s = UnityEngine.PlayerPrefs.GetInt("St_PG"..uid)
    if s == nil or s == 0 then
        s = 1
    end
    return s
...
function SettingViewModel.SetSkinState(value)
    local uid = PlayerControl.GetPlayerData().UID
    UnityEngine.PlayerPrefs.SetInt("St_Skin"..uid,value)
end

function SettingViewModel.GetSkinState()
    local uid = PlayerControl.GetPlayerData().UID
    local s = UnityEngine.PlayerPrefs.GetInt("St_Skin"..uid)
    if s == nil or s == 0 then
        s = 1
    end
    return s
...
---设置主界面是否保持当前状态(默认0关闭)
function SettingViewModel.SetHoldSpine(value)
    local isHold = value and 1 or 0
    UnityEngine.PlayerPrefs.SetInt(PlayerControl.GetPlayerData().UID.."St_HoldSpine",isHold)
end
function SettingViewModel.GetHoldSpine()
    local isHold = UnityEngine.PlayerPrefs.GetInt(PlayerControl.GetPlayerData().UID.."St_HoldSpine",1) == 1
    return isHold
end
