-- Code Auto Create Begin
local M = Class('UsmPlay', UIBase)
function M:ctor()
    M.super.ctor(self)
    self.Uid = UID.UsmPlay
    self.PathPrefab = 'ABOriginal/Prefab/Form/Form[UsmPlay].prefab'
    self.Name = 'Form[UsmPlay]'
    self.Layer = UILayerLv.Pop
    self.Depth = 1
    -- 没有使用组建缓存列表
    self.CC = {
        -- Image 列表
        {'SwitchPanelVideoPlayer','SwitchPanelVideoPlayer',2},
    }
end
-- Code Auto Create End
function M:OnInit()

end

function M:OnShow(pData)
    self.usmName = pData[1]
    self.callBack = pData[2]
    local criUI = self.SwitchPanelVideoPlayer().transform:GetComponent("CriManaMovieControllerForUI")
    criUI.player:SetFile(nil,MgrRes.GetABPath("USM/Common/Common/"..self.usmName..".usm"))               --给组件的player设置usm视频文件
    criUI.player.statusChangeCallback = function(state)
        if tostring(state) == "PlayEnd: 6" then    --播放完
            if self.callBack then
                self.callBack()
            end
            criUI.player.statusChangeCallback = nil
            MgrUI.ClosePop(self.Uid)
        end
    end
    criUI:Play()            --播放视频
end

return M