...
function PostMailViewModel.GetDayKeyByUnixTime(unixTime,hour)
    ...
    local retStr = os.date("%Y-%m-%d %H:%M:%S",unixTime + (tonumber(SteamLocalData.tab[112007][2]) - Global.GetClientTimeZone()) * 3600)
    local time = unixTime
    local data = os.date("!*t",time + tonumber(SteamLocalData.tab[112007][2]) * 3600)
    --(hour)4点前按前一天算
    if data.hour < hour then
        time = time - 24*60*60
    end
    local data2 = os.date("!*t",time + tonumber(SteamLocalData.tab[112007][2]) * 3600)
    --dump(data2)
    data2.hour = 0
    data2.min = 0
    data2.sec = 0
