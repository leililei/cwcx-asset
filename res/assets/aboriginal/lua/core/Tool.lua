...
-- 删除列表的值
---@generic TValue
---@param list TValue[] @ 数据列表，该列表会被修改
---@param value TValue
---@param bRemoveAll boolean @ 移除全部该值
---@return int @ 删除个数
function table.RemoveElementByValue(list, value, bRemoveAll)
    local rmCount = 0
    for i = #list, 1, -1 do
...
function Tool.Hex2Color(hex)
    local r = hex >> 16;
    local g = (hex & 0xFF00) >> 8;
    local b = hex & 0xFF;
    return Color(r / 255, g / 255, b / 255, 1);
end

local PRINT_PREFIX = "";
--local PRINT_PREFIX = "[XPXP] ";
local PRINT_STACK = false;
function Tool.print(...)
    local args = {...};
    local str = PRINT_PREFIX;
    for i = 1, #args, 1 do
        if nil == args[i] then
            str = str .. "nil ";
        elseif true == args[i] then
            str = str .. "true ";
        elseif false == args[i] then
            str = str .. "false ";
        elseif "function" == type(args[i]) then
            str = str .. "[function] ";
        elseif "table" == type(args[i]) then
            str = str .. "[table] ";
        else
            str = str .. args[i] .. " ";
        end
    end
    if true == PRINT_STACK then
        str = str .. "\n" .. debug.traceback();
    end
    print(str);
end
