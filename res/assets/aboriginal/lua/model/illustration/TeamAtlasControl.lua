...
function TeamAtlasControl.ScreenTeamData()
    local arr = {}
    RegionData = {}
    for k,v in ipairs(AllTeamData) do
        ...
        if arr[v.AreaType] ~= nil then
            ...
            if v:GetShowState() == true then
                if v.AreaSort ~= (#arr[v.AreaType]+1) then
                    v.AreaSort = #arr[v.AreaType]+1
                end
...
---传入id获取角色语音数据
function TeamAtlasControl.GetVoiceData(_roleData,_skinData)
    local arr = {}
    local hero = _roleData
    local skin = _skinData
    if skin == nil then
        skin = HeroControl.GetSkinDataBySkinId(hero.skin)
    end
    ...
    for i, n in pairs(ActorLinesLocalData.tab) do
        ...
        if n[2] == tonumber(skin.interaction) then      --匹配当前角色的语音
            if string.sub(n[5], 1, 1) == "1" then
                ...
                local t =
                {
                    favor =  (str[3] ~= nil and tonumber(str[3]) or 0),
                    type = n[3],
                    jpPath = n[13],
                    id = n[1],
                    isLock = false
                }
                if hero.favor >= (str[3] ~= nil and tonumber(str[3]) or 0) then
                    t.isLock = true
                else
                    t.isLock = false
                end
